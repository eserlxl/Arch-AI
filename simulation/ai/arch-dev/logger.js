let ARCH = function (m) {
	...
	m.Logger.prototype.print = function (type, text) {
	...
	m.Logger.prototype.push = function (type, section, data, initialValue = false) {
		...
		let dataHash = m.hash(data, 20); // Max. fixed debug message length was considered as 20

		if (this.log[section][dataHash]) {
			this.severity[section][dataHash] = m.limit(this.severity[section][dataHash] * 0.95, 1, 1000);
		} else {
	...
	m.Logger.prototype.update = function () {
