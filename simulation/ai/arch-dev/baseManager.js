...
ARCH.BaseManager = function (gameState, Config) {
	this.Config = Config;
	...
	this.requiredDefenders = this.Config.Defence.expectedDefenders;
...
ARCH.BaseManager.prototype.init = function (gameState, state) {
	if (state === "unconstructed")
		this.constructing = true;
	else if (state !== "captured")
		this.requiredDefenders = 0;
	this.workerObject = new ARCH.Worker(this);
	// entitycollections
...
ARCH.BaseManager.prototype.reset = function (gameState, state) {
	this.constructing = state === "unconstructed";
...
ARCH.BaseManager.prototype.assignEntity = function (gameState, ent) {
...
ARCH.BaseManager.prototype.setAnchor = function (gameState, anchorEntity) {
...
/// DEBUG
		...
		gameState.ai.logger.push("ERROR", "BaseManager", "Base " + this.ID + " has been assigned " + ent.templateName() + " as anchor.");
	}
/// DEBUG
	anchorEntity.setMetadata(PlayerID, "base", this.ID);
	this.buildings.updateEnt(anchorEntity);
	this.accessIndex = ARCH.getLandAccess(gameState, anchorEntity);
	return true;
};

/* we lost our anchor. Let's reaffect our units and buildings */
ARCH.BaseManager.prototype.anchorLost = function (gameState, ent) {
	this.anchor = undefined;
	this.anchorId = undefined;
	this.requiredDefenders = 0;
	gameState.ai.HQ.resetBaseCache();
...
ARCH.BaseManager.prototype.setAnchorlessEntity = function (gameState, ent) {
	if (!this.buildings.hasEntities()) {
/// DEBUG
		if (!ARCH.getBuiltEntity(gameState, ent).resourceDropsiteTypes()) {
			gameState.ai.logger.push("ERROR", "BaseManager", "Base " + this.ID + " has been assigned " + ent.templateName() + " as origin.");
		}
/// DEBUG
		this.accessIndex = ARCH.getLandAccess(gameState, ent);
	}
/// DEBUG
	else if (this.accessIndex !== ARCH.getLandAccess(gameState, ent)) {
		gameState.ai.logger.push("ERROR", "BaseManager", "Base " + this.ID + " with access " + this.accessIndex + " has been assigned " + ent.templateName() + " with access" + ARCH.getLandAccess(gameState, ent));
	}
/// DEBUG
	ent.setMetadata(PlayerID, "base", this.ID);
	this.buildings.updateEnt(ent);
 ...
 */
ARCH.BaseManager.prototype.assignResourceToDropsite = function (gameState, dropsite) {
	if (this.dropsites[dropsite.id()]) {
/// DEBUG
		gameState.ai.logger.push("ERROR", "BaseManager", "assignResourceToDropsite: dropsite already in the list. Should never happen");
/// DEBUG
		return;
	}
	...
	if (this.ID === gameState.ai.HQ.baseManagers[0].ID)
		accessIndex = ARCH.getLandAccess(gameState, dropsite);
	...
	for (let type of dropsite.resourceDropsiteTypes()) {
		...
		resources.forEach(function (supply) {
			...
			if (ARCH.getLandAccess(gameState, supply) !== accessIndex)
				return;
...
ARCH.BaseManager.prototype.removeDropsite = function (gameState, ent) {
	if (!ent.id())
		return;
 ...
 */
ARCH.BaseManager.prototype.findBestDropsiteLocation = function (gameState, resource) {
	...
	let obstructions = ARCH.createObstructionMap(gameState, this.accessIndex, template);
...
ARCH.BaseManager.prototype.getResourceLevel = function (gameState, type, nearbyOnly = false) {
	let count = 0;
	let check = {};
...
ARCH.BaseManager.prototype.checkResourceLevels = function (gameState, queues) {
		...
		if (total > 150 || total > 60 && type !== "wood") {
			...
			if (ratio > 0.15) {
				...
				if (newDP.quality > 50 && gameState.ai.HQ.canBuild(gameState, "structures/{civ}_storehouse"))
					queues.dropsites.addPlan(new ARCH.ConstructionPlan(gameState, "structures/{civ}_storehouse", {
						"base": this.ID,
						"type": type
					}, newDP.pos));
					...
					if (!gameState.ai.HQ.buildNewBase(gameState, queues, type) &&
...
ARCH.BaseManager.prototype.addGatherRates = function (gameState, currentRates) {
...
ARCH.BaseManager.prototype.assignRolelessUnits = function (gameState, roleless) {
	if (!roleless)
 ...
 */
ARCH.BaseManager.prototype.setWorkersIdleByPriority = function (gameState) {
 ...
 */
ARCH.BaseManager.prototype.switchGatherer = function (gameState, from, to, number) {
	let num = number;
	let only;
...
ARCH.BaseManager.prototype.reassignIdleWorkers = function (gameState, idleWorkers, requiredResource = false) {
	...
	for (let ent of idleWorkers) {
		...
		if (ent.hasClass("Worker")) {
			...
			if (ent.isBuilder() && this.anchor && this.anchor.needsRepair() &&
				...
				let mostNeeded = gameState.ai.HQ.pickMostNeededResources(gameState, requiredResource);
...
ARCH.BaseManager.prototype.workersBySubrole = function (gameState, subrole) {
...
ARCH.BaseManager.prototype.gatherersByType = function (gameState, type) {
 ...
 */
ARCH.BaseManager.prototype.pickBuilders = function (gameState, workers, number) {
	let availableWorkers = this.workers.filter(ent => {
		if (!ent.position() || !ent.isBuilder())
			return false;
		if (ent.getMetadata(PlayerID, "plan") === -2 || ent.getMetadata(PlayerID, "plan") === -3)
			return false;
		return !ent.getMetadata(PlayerID, "transport");
 ...
 */
ARCH.BaseManager.prototype.assignToFoundations = function (gameState, noRepair) {
	...
	for (let target of foundations.values()) {
		...
		if (ARCH.isNotWorthBuilding(gameState, target))
			continue;
...
ARCH.BaseManager.prototype.update = function (gameState, queues, events) {
	if (this.ID === gameState.ai.HQ.baseManagers[0].ID)	// base for unaffected units
	{
		...
		if (gameState.ai.HQ.numActiveBases() > 0) {
			for (let ent of this.units.values()) {
				let bestBase = ARCH.getBestBase(gameState, ent);
...
/// DEBUG
					if (ent.hasClass("Dock")) {
						gameState.ai.logger.push("DEBUG", "BaseManager", "Dock in baseManager[0]. It may be useful to do an anchorless base for " + ent.templateName());
					}
/// DEBUG
					continue;
				}
	...
	if (!this.anchor)   // This anchor has been destroyed, but the base may still be usable
	{
		if (!this.buildings.hasEntities()) {
			...
			for (let ent of this.units.values()) {
				let base = ARCH.getBestBase(gameState, ent, false, this.ID);
		...
		for (let ent of this.buildings.values()) {
			if (!ent.position())
				continue;
			let base = ARCH.getBestBase(gameState, ent);
			if (base.anchor)
				reassignedBase = base;
			break;
		}
...
///
	...
	} else if (this.requiredDefenders && gameState.ai.HQ.trainingManager.emergencyDefender(gameState, [this.anchor.position()]))
		--this.requiredDefenders;
...
///
ARCH.BaseManager.prototype.Serialize = function () {
	return {
		"ID": this.ID,
		...
		"requiredDefenders": this.requiredDefenders,
		"territoryIndices": this.territoryIndices,
		"timeNextIdleCheck": this.timeNextIdleCheck
	};
};

ARCH.BaseManager.prototype.Deserialize = function (gameState, data) {
	for (let key in data)
		this[key] = data[key];
