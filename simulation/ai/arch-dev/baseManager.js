...
/// DEBUG
			...
			gameState.ai.logger.push("ERROR", "BaseManager", "Base " + this.ID + " with access " + this.accessIndex + " has been assigned " + ent.templateName() + " with access" + m.getLandAccess(gameState, ent));
		}
/// DEBUG
		ent.setMetadata(PlayerID, "base", this.ID);
		this.buildings.updateEnt(ent);
...
// completely remove the dropsite resources from our list.
			...
			if (total > 150 || total > 60 && type !== "wood") {
				...
				if (ratio > 0.15) {
					...
					if (newDP.quality > 50 && gameState.ai.HQ.canBuild(gameState, "structures/{civ}_storehouse"))
						queues.dropsites.addPlan(new m.ConstructionPlan(gameState, "structures/{civ}_storehouse", {
							"base": this.ID,
							"type": type
						}, newDP.pos));
	...
	m.BaseManager.prototype.reassignIdleWorkers = function (gameState, idleWorkers, requiredResource = false) {
