...
ARCH.TERRITORY_PLAYER_MASK = 0x1F;
ARCH.TERRITORY_BLINKING_MASK = 0x40;

ARCH.createObstructionMap = function (gameState, accessIndex, template) {
	...
	for (let k = 0; k < territoryMap.data.length; ++k) {
		let tilePlayer = territoryMap.data[k] & ARCH.TERRITORY_PLAYER_MASK;
		let isConnected = (territoryMap.data[k] & ARCH.TERRITORY_BLINKING_MASK) === 0;
...
ARCH.createTerritoryMap = function (gameState) {
	let map = gameState.ai.territoryMap;

	let ret = new API3.Map(gameState.sharedScript, "territory", map.data);
	ret.getOwner = function (p) {
		return this.point(p) & ARCH.TERRITORY_PLAYER_MASK;
	};
	ret.getOwnerIndex = function (p) {
		return this.map[p] & ARCH.TERRITORY_PLAYER_MASK;
	};
	ret.isBlinking = function (p) {
		return (this.point(p) & ARCH.TERRITORY_BLINKING_MASK) !== 0;
	};
	return ret;
};
...
ARCH.outside_Mask = 1;
ARCH.border_Mask = 2;
ARCH.fullBorder_Mask = ARCH.outside_Mask | ARCH.border_Mask;
ARCH.narrowFrontier_Mask = 4;
ARCH.largeFrontier_Mask = 8;
ARCH.fullFrontier_Mask = ARCH.narrowFrontier_Mask | ARCH.largeFrontier_Mask;

ARCH.createBorderMap = function (gameState) {
...
ARCH.debugMap = function (gameState, map) {
	let width = map.width;
