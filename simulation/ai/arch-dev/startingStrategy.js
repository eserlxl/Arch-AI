...
ARCH.HQ.prototype.gameAnalysis = function (gameState) {
	...
	let nobase = new ARCH.BaseManager(gameState, this.Config);
 ...
 */
ARCH.HQ.prototype.assignStartingEntities = function (gameState) {
...
/// DEBUG
		...
		if (!bestbase)	// entity outside our territory
		{
			if (ent.hasClass("Structure") && !ent.decaying() && ent.resourceDropsiteTypes())
				...
				bestbase = ARCH.getBestBase(gameState, ent) || this.baseManagers[0];
 ...
 */
ARCH.HQ.prototype.regionAnalysis = function (gameState) {
 ...
 */
ARCH.HQ.prototype.structureAnalysis = function (gameState) {
 ...
 */
ARCH.HQ.prototype.buildFirstBase = function (gameState) {
	...
	if (!total.canAfford(new API3.Resources(template.cost()))) {
		...
		for (let ent of gameState.getOwnUnits().values()) {
			...
			if (ent.isIdle())
				ARCH.gatherTreasure(gameState, ent);
	...
	if (goal === "dock") {
		...
		gameState.ai.queues.dock.addPlan(new ARCH.ConstructionPlan(gameState, "Dock", "structures/{civ}_dock", {
			"sea": sea,
			"proximity": startingPoint[imax].pos
		}));
	} else
		gameState.ai.queues.civilCentre.addPlan(new ARCH.ConstructionPlan(gameState, "CivCentre", "structures/{civ}_civil_centre", {
			"base": -1,
			"resource": "wood",
			"proximity": startingPoint[imax].pos
		}));
 ...
 */
ARCH.HQ.prototype.dispatchUnits = function (gameState) {
...
/// DEBUG
		...
		units.filter(API3.Filters.byClassesAnd(["Infantry", "Ranged"])).forEach(ent => {
			...
			for (let cc of allycc) {
				if (!cc.position() || ARCH.getLandAccess(gameState, cc) !== access)
					continue;
				--num;
				--num1;
 ...
 */
ARCH.HQ.prototype.configFirstBase = function (gameState) {
...
/// DEBUG
	...
	if (!gameState.getOwnEntitiesByClass("Storehouse", true).hasEntities() && this.canBuild(gameState, template)) {
		...
		if (newDP.quality > 40) {
			...
			gameState.ai.queues.dropsites.addPlan(new ARCH.ConstructionPlan(gameState, "Storehouse", template, {"base": this.baseManagers[1].ID}, newDP.pos));
		}
	}
};
