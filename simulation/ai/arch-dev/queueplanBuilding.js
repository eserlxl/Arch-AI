...
ARCH.ConstructionPlan = function (gameState, typeName, type, metadata, position) {
...
ARCH.ConstructionPlan.prototype = Object.create(ARCH.QueuePlan.prototype);

ARCH.ConstructionPlan.prototype.canStart = function (gameState) {
/// DEBUG
	gameState.ai.logger.push("DEBUG", "ConstructionPlan", "Construction plan can be started.");
/// DEBUG
...
ARCH.ConstructionPlan.prototype.start = function (gameState) {
///
	Engine.ProfileStart("Building construction start");
///
/// DEBUG
	gameState.ai.logger.push("DEBUG", "ConstructionPlan", "Starting the construction of " + this.type);
/// DEBUG
	...
	if (!builder) {
/// DEBUG
		gameState.ai.logger.push("WARNING", "ConstructionPlan", "Builder not found for the construction of " + this.type);
/// DEBUG
///
		Engine.ProfileStop();
///
...
///
/// DEBUG
		gameState.ai.logger.push("WARNING", "ConstructionPlan", "Couldn't find a suitable location for " + this.type);
/// DEBUG
		return;
	}
...
ARCH.ConstructionPlan.prototype.findGoodPosition = function (gameState) {
	let template = this.template;
/// DEBUG
	gameState.ai.logger.push("DEBUG", "ConstructionPlan", "Finding a better position for the construction.");
/// DEBUG
...
/// DEBUG
				...
				gameState.ai.logger.push("DEBUG", "ConstructionPlan", "Couldn't find a proper location for Civil Centre, trying to build a dock!");
			}
/// DEBUG
	...
	} else	// No position was specified so try to find a sensible place to build
	{
		...
		if (gameState.ai.HQ.Config.Base.plan[this.typeName] === 0) {

			if (this.typeName === "House" || this.typeName === "Field") {
				gain = 1;
			} else {
				gain = 1.5;
			}
			...
			gameState.getOwnStructures().forEach(function (ent) {
				...
				let sameType = {
					...
					"Dock": 750,
					"Farmstead": 250,
					"House": -200, // Inverse effect for houses.
					"Field": -150, // Inverse effect for fields.
					"Military": -150, // Inverse effect for military
					"Storehouse": 250,
					"Gates": 150,
					"Tower": 150
				};

				for (let type in sameType) {
					if (ent.hasClass(type) && template.hasClass(type)) {
						placement.addInfluence(x, z, 60 / cellSize, -sameType[type]);    // Same type of stores should be built a bit further away from each other.
						same = true;
						if (sameType[type] > 0) {
							gain = 2;
						} else {
				...
				if (!same) {
					...
					let couple = {
						"CivCentre": {"Field": 500},
						"Farmstead": {"Field": 150, "Corral": 150},
						"Military": {"StoneWall": 80, "Gates": 80, "Tower": 80},
						"Field": {"Farmstead": 150}
					};

					let influence = 0;
					for (let mainType in couple) {
						if (ent.hasClass(mainType)) {
							for (let pairType in couple[mainType]) {
								if (template.hasClass(pairType)) {
									influence = couple[mainType][pairType]; // Couples should be together
									break;
								} else {
									influence = Math.min(influence, -couple[mainType][pairType]); // Buildings should be built a bit further away from couples
								}
							}
							if (influence !== 0) {
								placement.addInfluence(x, z, 60 / cellSize, influence);
							}
						}
					}
					...
					let decouple = {
						...
						"Dock": {"BarterMarket": 750},
						"BarterMarket": {"Dock": 750},
						"Farmstead": {"House": 250, "Storehouse": 150},
						"Storehouse": {"House": 150, "Farmstead": 150, "Corral": 150},
						"Corral": {"House": 150, "Storehouse": 150},
						"Military": {"House": 150},
						"Gates": {"House": 150},
						"StoneWall": {"House": 150},
						"House": {
					...
					for (let mainType in decouple) {
						if (ent.hasClass(mainType)) {
							for (let pairType in decouple[mainType]) {
								if (template.hasClass(pairType)) {
									placement.addInfluence(x, z, 60 / cellSize, -decouple[mainType][pairType]);
									gain = 2;
									break;
								}
							}
				...
				if (!template.hasClass("Field")
					&& !(template.hasClass("StoneWall") || template.hasClass("Gates"))
					&& (ent.hasClass("StoneWall") || ent.hasClass("Gates"))) {
					placement.addInfluence(x, z, 60 / cellSize, -30);    // Buildings should be built a bit further away from Walls and Gates
				}

				if (template.hasClass("GarrisonFortress") && ent.hasClass("House")) {
					placement.addInfluence(x, z, 120 / cellSize, -150);
				} else if (template.hasClass("Military")) {
					placement.addInfluence(x, z, 40 / cellSize, -150);
				} else if (template.hasClass("Wonder")) {
					if (ent.hasClass("CivCentre")) {
						placement.addInfluence(x, z, 60 / cellSize, 100);
					} else if (ent.hasClass("Military")) {
						placement.addInfluence(x, z, 60 / cellSize, 180);
					}
				}
			});
		} else if (gameState.ai.HQ.Config.Base.plan[this.typeName] === 1) {
			gameState.getOwnStructures().forEach(function (ent) {
				let pos = ent.position();
				let x = Math.round(pos[0] / cellSize);
				let z = Math.round(pos[1] / cellSize);

				let struct = ARCH.getBuiltEntity(gameState, ent);
		...
		if (template.hasClass("Farmstead")) {
			for (let j = 0; j < placement.map.length; ++j) {
				...
				if (HQ.borderMap.map[j] & ARCH.fullBorder_Mask)
	...
	if (this.metadata && this.metadata.base !== undefined) {
		...
		for (let j = 0; j < placement.map.length; ++j) {
			if (HQ.basesMap.map[j] !== base)
				...
				if (favorBorder && HQ.borderMap.map[j] & ARCH.border_Mask)
					placement.set(j, placement.map[j] + 50);
				else if (disfavorBorder && !(HQ.borderMap.map[j] & ARCH.fullBorder_Mask))
	...
	let radius = 0;
	if (template.hasClass("Fortress") || template.hasClass("Workshop") || this.type === gameState.applyCiv("structures/{civ}_elephant_stables"))
	...
	let bestTile = placement.findBestTile(radius * gain, obstructions);
 ...
 */
ARCH.ConstructionPlan.prototype.findDockPosition = function (gameState, maxWater = 16) {
	let template = this.template;
	...
	for (let j = 0; j < territoryMap.length; ++j) {
		...
		if (oversea) {
			...
			for (let dock of docks.values()) {
				if (ARCH.getSeaAccess(gameState, dock) !== navalPassMap[i])
					continue;
		...
		if (gameState.ai.HQ.borderMap.map[j] & ARCH.fullBorder_Mask)
			score += 20;
 ...
 */
ARCH.ConstructionPlan.prototype.buildOverseaDock = function (gameState, template, pos = false) {
	...
	for (let i = 0; i < gameState.ai.accessibility.regionSize.length; ++i) {
		...
		let keep = true;
		for (let dock of docks.values()) {
			if (ARCH.getLandAccess(gameState, dock) !== i)
				continue;
			keep = false;
			break;
		}
		...
		let sea;
		for (let cc of ccEnts.values()) {
			let ccAccess = ARCH.getLandAccess(gameState, cc);
			if (ccAccess !== i) {
...
/// DEBUG
	if (!gameState.ai.HQ.navalMap) {
		gameState.ai.logger.push("ERROR", "ConstructionPlan", "findOverseaLand on a non-naval map??? we should never go there");
	}
/// DEBUG
...
ARCH.ConstructionPlan.prototype.getDockAngle = function (gameState, x, z, size) {
 ...
 */
ARCH.ConstructionPlan.prototype.checkDockPlacement = function (gameState, x, z, halfDepth, halfWidth, angle) {
...
ARCH.ConstructionPlan.prototype.isDockLocation = function (gameState, j, dimension, wantedLand, wantedSea) {
	let width = gameState.ai.HQ.territoryMap.width;
	let cellSize = gameState.ai.HQ.territoryMap.cellSize;
 ...
 */
ARCH.ConstructionPlan.prototype.getFrontierProximity = function (gameState, j) {
	...
	for (let a of around) {
		for (let i = 1; i < 5; ++i) {
			...
			if (jz < 0 || jz >= width)
				continue;
			if (borderMap.map[jx + width * jz] & ARCH.outside_Mask)
				continue;
 ...
 */
ARCH.ConstructionPlan.prototype.getResourcesAround = function (gameState, types, i, radius) {
...
ARCH.ConstructionPlan.prototype.onStart = function (gameState) {
...
///
ARCH.ConstructionPlan.prototype.Serialize = function () {
	return {
...
ARCH.ConstructionPlan.prototype.Deserialize = function (gameState, data) {
	for (let key in data)
		this[key] = data[key];
