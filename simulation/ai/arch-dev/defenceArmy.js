/** Armies used by the defence manager.
 ...
 * @return {boolean}
 */
ARCH.DefenceArmy = function (gameState, foeEntities, type) {
 ...
 */
ARCH.DefenceArmy.prototype.addFoe = function (gameState, enemyId, force) {
 ...
 */
ARCH.DefenceArmy.prototype.removeFoe = function (gameState, enemyId, enemyEntity) {
 ...
 */
ARCH.DefenceArmy.prototype.addOwn = function (gameState, id, force) {
...
ARCH.DefenceArmy.prototype.removeOwn = function (gameState, id, Entity) {
 ...
 */
ARCH.DefenceArmy.prototype.clear = function (gameState) {
	...
	if (nOwn > 0)
		...
		for (let struct of gameState.getAllyStructures().values()) {
			...
			if (!pos || !gameState.isPlayerMutualAlly(gameState.ai.HQ.territoryMap.getOwner(pos)))
				continue;
			if (ARCH.getLandAccess(gameState, struct) !== armyAccess)
				continue;
	...
	while (this.ownEntities.length > 0) {
		...
		if (ent) {
			...
			if (ent.healthLevel() < this.Config.garrisonHealthLevel.low &&
				gameState.ai.HQ.defenceManager.garrisonAttackedUnit(gameState, ent))
				continue;
...
ARCH.DefenceArmy.prototype.assignUnit = function (gameState, entID) {
	...
	for (let id of this.foeEntities) {
		...
		if (eEnt.hasClass("Unit") && eEnt.unitAIOrderData() && eEnt.unitAIOrderData().length &&
			eEnt.unitAIOrderData()[0].target && eEnt.unitAIOrderData()[0].target === entID) {   // being attacked  >>> target the unit
			idMin = id;
			break;
		}
		...
		if (this.assignedAgainst[id].length > 8 ||
			this.assignedAgainst[id].length > 5 && !eEnt.hasClass("Hero") && !ARCH.isSiegeUnit(eEnt))
			continue;
	...
	let ownIndex = ARCH.getLandAccess(gameState, ent);
	...
	if (ownIndex === foeIndex || ent.hasClass("Ship")) {
		...
		ent.attack(idFoe, ARCH.allowCapture(gameState, ent, foeEnt), queued);
	} else
		gameState.ai.HQ.navalManager.requireTransport(gameState, ent, ownIndex, foeIndex, foePosition);
	return true;
};

ARCH.DefenceArmy.prototype.getType = function () {
	return this.type;
};

ARCH.DefenceArmy.prototype.getState = function () {
 ...
 */
ARCH.DefenceArmy.prototype.merge = function (gameState, otherArmy) {
...
ARCH.DefenceArmy.prototype.needsDefenders = function (gameState) {
	let defenceRatio;
	let territoryOwner = gameState.ai.HQ.territoryMap.getOwner(this.foePosition);
	if (territoryOwner === PlayerID)
		defenceRatio = this.Config.Defence.defenceRatio.own;
	else if (gameState.isPlayerAlly(territoryOwner)) {
		defenceRatio = this.Config.Defence.defenceRatio.ally;
		...
		for (let p = 1; p < gameState.sharedScript.playersData.length; ++p)
			if (p !== territoryOwner && gameState.sharedScript.playersData[p].isAlly[territoryOwner])
				++numExclusiveAllies;
		defenceRatio /= 1 + 0.5 * Math.max(0, numExclusiveAllies - 1);
	} else
		defenceRatio = this.Config.Defence.defenceRatio.neutral;
	...
	if (this.foeStrength * defenceRatio <= this.ownStrength)
		return false;
	return this.foeStrength * defenceRatio - this.ownStrength;
};


/** if not forced, will only recalculate if on a different turn. */
ARCH.DefenceArmy.prototype.recalculatePosition = function (gameState, force) {
...
ARCH.DefenceArmy.prototype.recalculateStrengths = function (gameState) {
	this.ownStrength = 0;
	this.foeStrength = 0;
...
ARCH.DefenceArmy.prototype.evaluateStrength = function (ent, isOwn, remove) {
	if (!ent)
		return;
	...
	if (ent.hasClass("Structure")) {
		...
		entStrength = ARCH.getMaxStrength(ent);
...
ARCH.DefenceArmy.prototype.checkEvents = function (gameState, events) {
...
ARCH.DefenceArmy.prototype.update = function (gameState) {
	for (let entId of this.ownEntities) {
		let ent = gameState.getEntityById(entId);
		if (!ent)
			continue;
		...
		if (!orderData.length && !ent.getMetadata(PlayerID, "transport"))
			...
			if (target && !ARCH.allowCapture(gameState, ent, target))
...
///
ARCH.DefenceArmy.prototype.Serialize = function () {
	return {
		"ID": this.ID,
		"type": this.type,
...
ARCH.DefenceArmy.prototype.Deserialize = function (data) {
	for (let key in data)
		this[key] = data[key];
};
///
