let ARCH = function (m) {
	...
	m.AttackPlan.prototype.isAvailableUnit = function (gameState, ent, expectedHealthLevel = 0.8) {
		if (!ent.position()) {
/// DEBUG
...
/// DEBUG
	...
	m.AttackPlan.prototype.countDestroyers = function (gameState) {
...
/// DEBUG
	...
	m.AttackPlan.prototype.navalTargetFinder = function (gameState, playerEnemy) {
		let validTargets = gameState.getEntities(playerEnemy).filter(this.isValidTarget, this);
			...
			if (this.isStarted()) {
				...
				for (let base of gameState.ai.HQ.baseManagers) {
					if (!base.anchor || !base.anchor.position())
						continue;
					if (this.type !== "Naval" && m.getLandAccess(gameState, base.anchor) !== access)
						continue;
