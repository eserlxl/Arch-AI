ARCH.ConstructManager = function (Config) {
	this.Config = Config;
...
ARCH.ConstructManager.prototype.init = function (gameState) {
};

ARCH.ConstructManager.prototype.update = function (gameState, queues) {
...
ARCH.ConstructManager.prototype.setQueuePriority = function (gameState, targetQueue, gain = 1) {
...
ARCH.ConstructManager.prototype.immediateConstruct = function (gameState, targetQueue) {
	let i = 0;
...
ARCH.ConstructManager.prototype.canBuild = function (gameState, structure, buildManager, numActiveBases) {
...
ARCH.ConstructManager.prototype.queue = function (gameState, queues, type, template, metadata, emergency, queueName, position) {

	let plan = new ARCH.ConstructionPlan(gameState, type, template, metadata, position);
...
ARCH.ConstructManager.prototype.Building = function (gameState, queues) {
	...
	let limit = {
		...
		"BarterMarket": (this.currentPhase + this.baseCount - 1) * (ARCH.checkPhase(gameState, 2) && this.basicMilitaryBaseCount > 1), // Build two markets after having enough basic military bases
		...
		"Workshop": ARCH.checkPhase(gameState, 3),
		"Blacksmith": Math.min(2, this.currentPhase),
		"ElephantStables": this.currentPhase * ARCH.checkPhase(gameState, 2),
		"Fortress": this.currentPhase * ARCH.checkPhase(gameState, 3),
		"SentryTower": (this.currentPhase < 2) * Math.max(this.basicMilitaryBaseCount, 2),
		"StoneTower": Math.max(this.basicMilitaryBaseCount, 3),
		"Wonder": ARCH.checkPhase(gameState, 3)
	};
...
/// DEBUG
	...
	if (!this.dockBuildable && this.checkDock) {
		...
		if (metadata["Dock"]) {
			let tempPlan = new ARCH.ConstructionPlan(gameState, "Dock", template["Dock"], metadata["Dock"]);
...
/// DEBUG
	...
	if (maxReq >= 1 && this.count[maxReqType] < this.target[maxReqType]
		&& (this.resource["wood"] > this.Config.Economy.expectedResource["wood"] || ARCH.checkPhase(gameState, 2) || emergency[maxReqType])) {
/// DEBUG
...
ARCH.ConstructManager.prototype.checkSea = function (gameState) {
	this.checkDock = true;
	let metadata = [];
...
ARCH.ConstructManager.prototype.planBuildDock = function (gameState) {
	let ownEntities = [];
 ...
 */
ARCH.ConstructManager.prototype.AdvancedBuilding = function (gameState, queues) {
	...
	if (!nAdvanced || nAdvanced < this.bAdvanced.length && this.population > 110) {
		for (let advanced of this.bAdvanced) {
			...
			if (template.hasDefensiveFire() || template.trainableEntities(civ))
				queues.militaryBuilding.addPlan(new ARCH.ConstructionPlan(gameState, "AdvancedMilitary", advanced, {"militaryBase": true}));
			else	// not a military building, but still use this queue
				queues.militaryBuilding.addPlan(new ARCH.ConstructionPlan(gameState, "AdvancedBuilding", advanced));
			return;
		}
	}
};
...
///
ARCH.ConstructManager.prototype.Serialize = function () {
	return {
		"test": true
	};
};

ARCH.ConstructManager.prototype.Deserialize = function (gameState, data) {
	for (let key in data) {
		this[key] = data[key];
	}
};
