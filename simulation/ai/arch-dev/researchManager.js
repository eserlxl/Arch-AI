...
ARCH.ResearchManager = function (Config) {
	this.Config = Config;
};
 ...
 */
ARCH.ResearchManager.prototype.checkPhase = function (gameState, queues) {
	...
	if (!nextPhaseName) {
		return;
	} else if (this.Config.phaseCount < currentPhaseIndex + 1) {
		this.Config.phaseCount = currentPhaseIndex + 1;
	}
	...
	if (gameState.hasResearchers(nextPhaseName, true)) {
		...
		queues.majorTech.addPlan(new ARCH.ResearchPlan(gameState, nextPhaseName, true));
	}
};

ARCH.ResearchManager.prototype.researchPopulationBonus = function (gameState, queues) {
	if (queues.minorTech.hasQueuedUnits())
		return;

	let techs = gameState.findAvailableTech();
	for (let tech of techs) {
		if (!tech[1]._template.modifications || tech[1]._template.modifications.length === 0)
			continue;
		...
		if (tech[1]._template.modifications[0].value !== "Cost/PopulationBonus")
			continue;
		queues.minorTech.addPlan(new ARCH.ResearchPlan(gameState, tech[0]));
		break;
	}
};

ARCH.ResearchManager.prototype.researchTradeBonus = function (gameState, queues) {
	if (queues.minorTech.hasQueuedUnits())
		return;
...
ARCH.ResearchManager.prototype.researchCorral = function (gameState, queues) {
	if (queues.minorTech.hasQueuedUnits())
		return;
	...
	for (let tech of techs) {
		if (!tech[1]._template.modifications || !tech[1]._template.affects)
			continue;
		if (tech[0] === "gather_animals_stockbreeding"
			|| tech[0] === "speed_cavalry_01"
			|| tech[0] === "speed_cavalry_02") {
			queues.minorTech.addPlan(new ARCH.ResearchPlan(gameState, tech[0]));
			break;
		}
	}
...
ARCH.ResearchManager.prototype.researchWantedTechs = function (gameState, techs) {
...
ARCH.ResearchManager.prototype.researchPreferredTechs = function (gameState, techs) {
...
ARCH.ResearchManager.prototype.update = function (gameState, queues) {
	...
	if (techName) {
		if (techName.increasePriority) {
			...
			let plan = new ARCH.ResearchPlan(gameState, techName.name);
			plan.queueToReset = "minorTech";
			queues.minorTech.addPlan(plan);
		} else
			queues.minorTech.addPlan(new ARCH.ResearchPlan(gameState, techName.name));
		return;
	}
	...
	queues.minorTech.addPlan(new ARCH.ResearchPlan(gameState, pickRandom(techs)[0]));
};

/**
 * @return {number}
 */
ARCH.ResearchManager.prototype.CostSum = function (cost) {
	let costSum = 0;
	for (let res in cost)
		costSum += cost[res];
	return costSum;
...
///
ARCH.ResearchManager.prototype.Serialize = function () {
	return {};
};

ARCH.ResearchManager.prototype.Deserialize = function (data) {
};
///
