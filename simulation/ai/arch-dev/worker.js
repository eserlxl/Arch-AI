...
ARCH.Worker = function (base) {
	this.ent = undefined;
	this.base = base;
	this.baseID = base.ID;
};

ARCH.Worker.prototype.update = function (gameState, ent) {
		...
		if (subrole === "gatherer" || subrole === "hunter") {
			...
			if (plan.state === "boarding" && ent.position()) {
				...
				for (let structure of gameState.getOwnStructures().values()) {
					if (ARCH.getLandAccess(gameState, structure) !== plan.endIndex)
						continue;
					let resourceDropsiteTypes = ARCH.getBuiltEntity(gameState, structure).resourceDropsiteTypes();
				...
				if (!hasDropsite) {
					for (let unit of gameState.getOwnUnits().filter(API3.Filters.byClass("Support")).values()) {
						if (!unit.position() || ARCH.getLandAccess(gameState, unit) !== plan.endIndex)
							continue;
	...
	this.entAccess = ARCH.getLandAccess(gameState, ent);
					...
					if (gameState.isPlayerEnemy(territoryOwner)) {
						...
						for (let dropsite of foodDropsites.values()) {
							...
							if (owner !== PlayerID && (!dropsite.isSharedDropsite() || !gameState.isPlayerMutualAlly(owner)))
								continue;
							if (targetAccess !== ARCH.getLandAccess(gameState, dropsite))
								continue;
	...
	if (unitAIStateOrder === "COMBAT") {
		if (subrole === "fisher")
			...
			if (orderData && orderData.target) {
				let target = gameState.getEntityById(orderData.target);
				if (target && (!target.position() || ARCH.getLandAccess(gameState, target) !== this.entAccess)) {
					if (this.retryWorking(gameState, subrole))
						return;
					ent.stopMoving();
				}
			...
			if (orderData && orderData.target && orderData.attackType && orderData.attackType === "Capture") {
				...
				if (target && target.owner() > 0 && !gameState.isPlayerAlly(target.owner()))
					ent.attack(orderData.target, ARCH.allowCapture(gameState, ent, target));
			}
		}
		return;
	}
	...
	if (subrole === "gatherer") {
		if (ent.isIdle()) {
			...
			} else if (!ARCH.returnResources(gameState, ent))     // try to deposit resources
			{
			...
			if (dropsite && dropsite.position() && this.entAccess !== ARCH.getLandAccess(gameState, dropsite))
				ARCH.returnResources(gameState, this.ent);
		...
		// Unless we've been explicitly told to keep our role, make us idle.
		let target = gameState.getEntityById(ent.getMetadata(PlayerID, "target-foundation"));
		if (!target || target.foundationProgress() === undefined && target.needsRepair() === false) {
			...
			let goalAccess = ARCH.getLandAccess(gameState, target);
			let queued = ARCH.returnResources(gameState, ent);
		...
		} else	// Perform some sanity checks
		{
			if (unitAIStateOrder === "GATHER" || unitAIStateOrder === "RETURNRESOURCE") {
				...
				if (territoryOwner !== 0 && !gameState.isPlayerAlly(territoryOwner))  // player is its own ally
					...
					let dropsite = gameState.getEntityById(ent.unitAIOrderData()[0].target);
					if (dropsite && dropsite.position() && this.entAccess !== ARCH.getLandAccess(gameState, dropsite))
						ARCH.returnResources(gameState, ent);
				}
			}
		}
...
ARCH.Worker.prototype.retryWorking = function (gameState, subrole) {
	switch (subrole) {
		case "gatherer":
...
ARCH.Worker.prototype.startBuilding = function (gameState) {
	let target = gameState.getEntityById(this.ent.getMetadata(PlayerID, "target-foundation"));
	if (!target || target.foundationProgress() === undefined && target.needsRepair() === false)
		return false;
	if (ARCH.getLandAccess(gameState, target) !== this.entAccess)
		return false;
	this.ent.repair(target, target.hasClass("House"));  // autocontinue=true for houses
	return true;
};

ARCH.Worker.prototype.startGathering = function (gameState) {
	// First look for possible treasure if any
	if (ARCH.gatherTreasure(gameState, this.ent))
		return true;
	...
	let findSupply = function (ent, supplies) {
		...
		for (let i = 0; i < supplies.length; ++i) {
			...
			if (ARCH.IsSupplyFull(gameState, supplies[i].ent))
				continue;
			...
			if (!gatherRates[supplyType])
				continue;
			// check if available resource is worth one additional gatherer (except for farms)
	...
	let shouldBuild = this.ent.isBuilder() && foundations.some(function (foundation) {
		if (!foundation || ARCH.getLandAccess(gameState, foundation) !== this.entAccess)
			return false;
	...
	shouldBuild = this.ent.isBuilder() && foundations.some(function (foundation) {
		...
		let structure = gameState.getBuiltTemplate(foundation.templateName());
		if (structure.resourceDropsiteTypes() && structure.resourceDropsiteTypes().indexOf(resource) !== -1) {
			let foundationAccess = ARCH.getLandAccess(gameState, foundation);
...
/// DEBUG
	gameState.ai.logger.push("DEBUG", "Worker", "Worker with gather-type " + resource + " with nothing to gather ");
/// DEBUG
 ...
 */
ARCH.Worker.prototype.startHunting = function (gameState, position) {
	// First look for possible treasure if any
	if (!position && ARCH.gatherTreasure(gameState, this.ent))
		return true;
	...
	let hasFoodDropsiteWithinDistance = function (supplyPosition, supplyAccess, distSquare) {
		for (let dropsite of foodDropsites.values()) {
			...
			if (owner !== PlayerID && (!dropsite.isSharedDropsite() || !gameState.isPlayerMutualAlly(owner)))
				continue;
			if (supplyAccess !== ARCH.getLandAccess(gameState, dropsite))
				continue;
	...
	for (let supply of resources.values()) {
		...
		if (ARCH.IsSupplyFull(gameState, supply))
			continue;
		...
		let supplyAccess = ARCH.getLandAccess(gameState, supply);
...
ARCH.Worker.prototype.startFishing = function (gameState) {
	...
	let fisherSea = ARCH.getSeaAccess(gameState, this.ent);
	...
	let nearestDropsiteDist = function (supply) {
		...
		for (let dropsite of fishDropsites) {
			...
			if (owner !== PlayerID && (!dropsite.isSharedDropsite() || !gameState.isPlayerMutualAlly(owner)))
				continue;
			if (fisherSea !== ARCH.getSeaAccess(gameState, dropsite))
				continue;
...
ARCH.Worker.prototype.gatherNearestField = function (gameState, baseID) {
	...
	for (let field of ownFields.values()) {
		if (ARCH.IsSupplyFull(gameState, field))
			continue;
 ...
 */
ARCH.Worker.prototype.buildAnyField = function (gameState, baseID) {
 ...
 */
ARCH.Worker.prototype.moveToGatherer = function (gameState, ent, forced) {
	let pos = ent.position();
	if (!pos || ent.getMetadata(PlayerID, "target-foundation") !== undefined)
		return;

	let waitTime = ent.getMetadata(PlayerID, "nextMoveToGatherer");

	if (!waitTime) waitTime = 5;

	if (!forced && gameState.ai.elapsedTime < waitTime)
		return;
	let gatherers = this.base.workersBySubrole(gameState, "gatherer");
	let builders = this.base.workersBySubrole(gameState, "builder");
	let dist = Math.min();
	let destination;
	...
	for (let gatherer of gatherers.values()) {
		...
		if (!gatherer.position() || gatherer.getMetadata(PlayerID, "transport") !== undefined ||
			ARCH.getLandAccess(gameState, gatherer) !== access || gatherer.isIdle())
			continue;
	...
	let sameWork = 0;
	for (let builder of builders.values()) {
		if (ent !== builder && gameState.getEntityById(ent.getMetadata(PlayerID, "target-foundation")) === gameState.getEntityById(builder.getMetadata(PlayerID, "target-foundation"))) {
			sameWork++;
		}
	}

	if (sameWork) ent.setMetadata(PlayerID, "nextMoveToGatherer", gameState.ai.elapsedTime + 5);
	if (destination && dist > 0) // 10
 ...
 */
ARCH.Worker.prototype.isInaccessibleSupply = function (gameState) {
