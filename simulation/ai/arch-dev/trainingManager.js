ARCH.TrainingManager = function (Config) {
	this.Config = Config;
...
ARCH.TrainingManager.prototype.init = function (gameState) {
...
ARCH.TrainingManager.prototype.update = function (gameState, queues) {
...
/// DEBUG
	...
	if (this.mainWorkerCount > ARCH.limit(gameState.getPopulationMax() / 3, 50, 150)) {
...
ARCH.TrainingManager.prototype.decide = function (gameState, classes, requirements) {
	let units;
	...
	units.sort((a, b) => {
		...
		for (let param of parameters) {
			if (param[0] === "strength") {
				aValue += ARCH.getMaxStrength(a[1]) * param[1];
				bValue += ARCH.getMaxStrength(b[1]) * param[1];
			} else if (param[0] === "siegeStrength") {
				aValue += ARCH.getMaxStrength(a[1], "Structure") * param[1];
				bValue += ARCH.getMaxStrength(b[1], "Structure") * param[1];
 ...
 */
ARCH.TrainingManager.prototype.emergencyDefender = function (gameState, positions) {
	...
	for (let pos of positions) {
		...
		for (let base of gameState.ai.HQ.baseManagers) {
			if (!base.anchor || !base.anchor.position())
				continue;
			if (ARCH.getLandAccess(gameState, base.anchor) !== access)
				continue;
...
/// DEBUG
	...
	gameState.ai.queues.emergency.addPlan(new ARCH.TrainingPlan(gameState, templateFound[0], metadata, this.currentPhase, this.currentPhase));
	return true;
};

ARCH.TrainingManager.prototype.train = function (gameState, queues, type, queueSize = 1, emergency = false, maxQueueCount = 1) {
...
/// DEBUG
	...
	queueObject.addPlan(new ARCH.TrainingPlan(gameState, trainTemplate[templateIndex][0], this.metaData[type], queueSize, queueSize));
...
///
ARCH.TrainingManager.prototype.Serialize = function () {
	return {
		"test": true
	};
};

ARCH.TrainingManager.prototype.Deserialize = function (gameState, data) {
	for (let key in data) {
		this[key] = data[key];
	}
};
