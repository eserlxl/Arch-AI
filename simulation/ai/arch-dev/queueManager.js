...
ARCH.QueueManager = function (Config, queues) {
	this.Config = Config;
	this.queues = queues;
	this.priorities = {};
...
ARCH.QueueManager.prototype.init = function (gameState) {
	this.logger = gameState.ai.logger;
};

ARCH.QueueManager.prototype.getAvailableResources = function (gameState) {
...
ARCH.QueueManager.prototype.getTotalAccountedResources = function () {
...
ARCH.QueueManager.prototype.currentNeeds = function (gameState) {
...
ARCH.QueueManager.prototype.wantedGatherRates = function (gameState, requiredResource = false) {
...
/// DEBUG
ARCH.QueueManager.prototype.printQueues = function (gameState) {
...
ARCH.QueueManager.prototype.clear = function () {
	for (let i in this.queues)
		this.queues[i].empty();
};
 ...
 */
ARCH.QueueManager.prototype.setAccounts = function (gameState, cost, i) {
 ...
 */
ARCH.QueueManager.prototype.transferAccounts = function (cost, i, j) {
	for (let res of Resources.GetCodes()) {
 ...
 */
ARCH.QueueManager.prototype.distributeResources = function (gameState) {
...
ARCH.QueueManager.prototype.switchResource = function (gameState, res) {
...
ARCH.QueueManager.prototype.startNextItems = function (gameState) {
...
ARCH.QueueManager.prototype.update = function (gameState) {
///
...
ARCH.QueueManager.prototype.checkPausedQueues = function (gameState) {
...
ARCH.QueueManager.prototype.canAfford = function (queue, cost) {
	if (!this.accounts[queue])
		return false;
	return this.accounts[queue].canAfford(cost);
};

ARCH.QueueManager.prototype.pauseQueue = function (queue, scrapAccounts) {
...
ARCH.QueueManager.prototype.unpauseQueue = function (queue) {
	if (this.queues[queue])
		this.queues[queue].paused = false;
};

ARCH.QueueManager.prototype.pauseAll = function (scrapAccounts, but) {
	for (let q in this.queues) {
		if (q === but)
			continue;
		if (scrapAccounts)
...
ARCH.QueueManager.prototype.unpauseAll = function (but) {
	for (let q in this.queues)
		if (q !== but)
...
ARCH.QueueManager.prototype.addQueue = function (queueName, priority) {
	if (this.queues[queueName] !== undefined)
		return;

	this.queues[queueName] = new ARCH.Queue();
...
ARCH.QueueManager.prototype.removeQueue = function (queueName) {
	if (this.queues[queueName] === undefined)
		return;
...
ARCH.QueueManager.prototype.getPriority = function (queueName) {
	return this.priorities[queueName];
};

ARCH.QueueManager.prototype.changePriority = function (queueName, newPriority) {
/// DEBUG
...
ARCH.QueueManager.prototype.sortQueues = function () {
	let priorities = this.priorities;
	this.queueArrays.sort((a, b) => priorities[b[0]] - priorities[a[0]]);
};

///
ARCH.QueueManager.prototype.Serialize = function () {
	let accounts = {};
	let queues = {};
	for (let q in this.queues) {
...
ARCH.QueueManager.prototype.Deserialize = function (gameState, data) {
