...
ARCH.GarrisonManager = function (Config) {
	this.Config = Config;
	this.holders = new Map();
	this.decayingStructures = new Map();
};

ARCH.GarrisonManager.prototype.update = function (gameState, events) {
...
/// DEBUG
					gameState.ai.logger.push("ERROR", "GarrisonManager", "unit " + ent.id() + " (" + ent.genericName() +
...
/// DEBUG
		...
		if (gameState.ai.elapsedTime - holder.getMetadata(PlayerID, "holderTimeUpdate") > 3) {
			...
			let around = {"defenceStructure": false, "meleeSiege": false, "rangeSiege": false, "unit": false};
			for (let ent of gameState.getEnemyEntities().values()) {
				...
				if (dist > range * range)
					continue;
				if (ent.hasClass("Structure"))
					around.defenceStructure = true;
				else if (ARCH.isSiegeUnit(ent)) {
			...
			// Keep defenceManager.garrisonUnitsInside in sync to avoid garrisoning-ungarrisoning some units
			data.allowMelee = around.defenceStructure || around.unit;
	...
	for (let [id, gmin] of this.decayingStructures.entries()) {
		...
		if (!ent || ent.owner() !== PlayerID)
			...
			gameState.ai.HQ.defenceManager.garrisonUnitsInside(gameState, ent, {"min": gmin, "type": "decay"});
	}
};

/** TODO should add the units garrisoned inside garrisoned units */
ARCH.GarrisonManager.prototype.numberOfGarrisonedUnits = function (holder) {
...
ARCH.GarrisonManager.prototype.allowMelee = function (holder) {
	if (!this.holders.has(holder.id()))
		return undefined;
...
ARCH.GarrisonManager.prototype.garrison = function (gameState, ent, holder, type) {
...
/// DEBUG
	gameState.ai.logger.push("DEBUG", "GarrisonManager", "Garrisoning unit " + ent.genericName() + " in " + holder.genericName() + " with type " + type);
	gameState.ai.logger.push("DEBUG", "GarrisonManager", "Trying to garrison a unit with plan " + ent.getMetadata(PlayerID, "plan") + " and role " + ent.getMetadata(PlayerID, "role") +
 ...
 */
ARCH.GarrisonManager.prototype.leaveGarrison = function (ent) {
...
ARCH.GarrisonManager.prototype.cancelGarrison = function (ent) {
	ent.stopMoving();
	this.leaveGarrison(ent);
...
ARCH.GarrisonManager.prototype.keepGarrisoned = function (ent, holder, around) {
	switch (ent.getMetadata(PlayerID, "garrisonType")) {
		case 'force':           // force the ungarrisoning
			return false;
		case 'trade':		// trader garrisoned in ship
			return true;
		case 'protection':	// hurt unit for healing or infantry for defence
			...
			if (MatchesClassList(ent.classes(), holder.getGarrisonArrowClasses())) {
				if (around.unit || around.defenceStructure)
					return true;
			...
			if (ent.attackTypes() && ent.attackTypes().indexOf("Melee") !== -1)
				return false;
			if (around.unit)
				return ent.hasClass("Support") || ARCH.isSiegeUnit(ent);	// only ranged siege here and below as melee siege already released above
		...
		case 'emergency': // f.e. hero in regicide mode
			if (holder.buffHeal() && ent.isHealable() && ent.healthLevel() < this.Config.garrisonHealthLevel.high)
				return true;
			if (around.unit || around.defenceStructure || around.meleeSiege ||
...
/// DEBUG
			gameState.ai.logger.push("ERROR", "GarrisonManager", "Unknown garrison type: " + ent.getMetadata(PlayerID, "garrisonType") +
...
ARCH.GarrisonManager.prototype.registerHolder = function (gameState, holder) {
 ...
 * do it only for structures useful for defence, except if we are expanding (justCaptured=true)
 * in which case we also do it for structures useful for unit trainings (TODO only Barracks are done)
 */
ARCH.GarrisonManager.prototype.addDecayingStructure = function (gameState, entId, justCaptured) {
...
ARCH.GarrisonManager.prototype.removeDecayingStructure = function (entId) {
	if (!this.decayingStructures.has(entId))
		return;
...
///
ARCH.GarrisonManager.prototype.Serialize = function () {
	return {"holders": this.holders, "decayingStructures": this.decayingStructures};
};

ARCH.GarrisonManager.prototype.Deserialize = function (data) {
	for (let key in data)
		this[key] = data[key];
};
///
