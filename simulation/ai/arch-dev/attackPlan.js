...
ARCH.AttackPlan = function (gameState, Config, uniqueID, type, data) {
	this.Config = Config;
	this.name = uniqueID;
...
/// DEBUG
	...
	for (let base of gameState.ai.HQ.baseManagers) {
		if (!base.anchor || !base.anchor.position())
			continue;
		let access = ARCH.getLandAccess(gameState, base.anchor);
		if (!rallyPoint) {
	...
	if (this.type !== "Naval" && gameState.ai.HQ.navalMap) {
		for (let structure of gameState.getEnemyStructures().values()) {
			...
			if (!structure.position())
				continue;
			let access = ARCH.getLandAccess(gameState, structure);
...
ARCH.AttackPlan.prototype.init = function (gameState, unitCollection = undefined) {
...
ARCH.AttackPlan.prototype.updatePreparation = function (gameState) {
	...
	for (let ent of this.unitCollection.values()) {
		...
		if (this.type === "Naval") {
			...
			if (ent.resourceCarrying() && ent.resourceCarrying().length)
				queued = ARCH.returnResources(gameState, ent);
...
ARCH.AttackPlan.prototype.assignShips = function (gameState) {
/// DEBUG
...
ARCH.AttackPlan.prototype.assignUnits = function (gameState) {
/// DEBUG
 ...
 * @return {boolean}
 */
ARCH.AttackPlan.prototype.StartAttack = function (gameState) {
/// DEBUG
...
/// DEBUG
		...
		let targetAccess = ARCH.getLandAccess(gameState, this.target);
...
ARCH.AttackPlan.prototype.update = function (gameState, events) {
...
///
	...
	if (this.state === "") {
		...
		for (let evt of events.Attacked) {
			...
			if (this.type !== "Naval") {
				if (ARCH.isSiegeUnit(ourUnit)) {	// if our siege units are attacked, we'll send some units to deal with enemies.
					...
					for (let ent of collec.values()) {
						if (ARCH.isSiegeUnit(ent))	// needed as mauryan elephants are not filtered out
							continue;
					...
					if (ARCH.isSiegeUnit(attacker) && attacker.hasClass("Melee") && ourUnit.hasClass("Melee")) {
						ourUnit.attack(attacker.id(), ARCH.allowCapture(gameState, ourUnit, attacker));
					...
					} else if (ARCH.isSiegeUnit(attacker)) {	// if our unit is attacked by a siege unit, we'll send some melee units to help it.
		...
		for (let ent of this.unitCollection.values()) {
			...
			if (!(targetId in unitTargets)) {
				if (this.type !== "Naval") {
					...
					if (ARCH.isSiegeUnit(target) || target.hasClass("Hero"))
		...
		for (let check = 0; check < lgth; check++) {
			...
			let siegeUnit = ARCH.isSiegeUnit(ent);
			if (ent.isIdle())
				needsUpdate = true;
			...
			if (this.isBlocked) {
				...
				range += 30;
			range = range * range;
			let entAccess = ARCH.getLandAccess(gameState, ent);
			...
			if (siegeUnit) {
				let mStruct = enemyStructures.filter(enemy => {
					...
					if (enemy.foundationProgress() === 0)
						return false;
					return ARCH.getLandAccess(gameState, enemy) === entAccess;
				...
				if (mStruct.length) {
					...
					if (mStruct[0].hasClass("Gates"))
						ent.attack(mStruct[0].id(), ARCH.allowCapture(gameState, ent, mStruct[0]));
					else {
						let rand = randIntExclusive(0, mStruct.length * 0.2);
						ent.attack(mStruct[rand].id(), ARCH.allowCapture(gameState, ent, mStruct[rand]));
					}
				} else {
				...
				let mUnit = enemyUnits.filter(enemy => {
					...
					if (dist > range)
						return false;
					if (ARCH.getLandAccess(gameState, enemy) !== entAccess)
						return false;
				...
				if (mUnit.length) {
					...
					ent.attack(mUnit[rand].id(), ARCH.allowCapture(gameState, ent, mUnit[rand]));
...
///
ARCH.AttackPlan.prototype.Serialize = function () {
	let properties = {
		"name": this.name,
		"type": this.type,
		"state": this.state,
...
ARCH.AttackPlan.prototype.Deserialize = function (gameState, data) {
