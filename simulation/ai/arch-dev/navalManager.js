...
ARCH.NavalManager = function (Config) {
	...
	this.breakBlockageCheckTurn = 0;
};

ARCH.NavalManager.prototype.updateNavy = function (gameState, init = false) {
	// docks
	...
	for (let i = 0; i < gameState.ai.accessibility.regionSize.length; ++i) {
		this.wantedTransportShips[i] = this.Config.Navy.targetNumTransportships;
		this.wantedWarShips[i] = this.Config.Navy.targetNumWarships;
		this.wantedFishShips[i] = this.Config.Navy.targetNumFishers;
	}
	...
	// Sometimes, ships can block each other and when they try to avoid this blockage, they do the same movement infinitely.
	// Therefore, we should stop ships randomly to break this blockage
	//let shipLoc = [];
	if (gameState.ai.playedTurn > this.breakBlockageCheckTurn + 60)
		for (let ship1 of gameState.getOwnUnits().values()) {
			if (!ship1.hasClass("Ship")) continue;
			for (let ship2 of gameState.getOwnUnits().values()) {
				if (!ship2.hasClass("Ship")) continue;
				if (ship1 !== ship2) {
					let distance = API3.VectorDistance(ship1.position(), ship2.position());
					//shipLoc.push({"distance": distance});
					if (distance < 10) {
						ship2.moveApart(ship2.position(), randFloat(10, 20));
						this.breakBlockageCheckTurn = gameState.ai.playedTurn;
						break;
					}
				}
			}
...
ARCH.NavalManager.prototype.init = function (gameState, deserializing) {

	this.updateNavy(gameState, true);
	...
	for (let i = 0; i < gameState.ai.accessibility.regionSize.length; ++i) {
		if (!gameState.ai.HQ.navalRegions[i]) {
			...
			this.wantedTransportShips.push(this.Config.Navy.targetNumTransportships);
			this.wantedWarShips.push(this.Config.Navy.targetNumWarships);
			if (availableFishes[i] && availableFishes[i] > 1000)
				this.wantedFishShips.push(this.Config.Navy.targetNumFishers);
			else
				this.wantedFishShips.push(0);
			this.neededTransportShips.push(0);
			this.neededWarShips.push(0);
	...
	for (let ship of this.ships.values())
		ARCH.setSeaAccess(gameState, ship);
	for (let dock of this.docks.values())
		ARCH.setSeaAccess(gameState, dock);
};

ARCH.NavalManager.prototype.updateFishingBoats = function (sea, num) {
	if (this.wantedFishShips[sea])
		this.wantedFishShips[sea] = num;
};

ARCH.NavalManager.prototype.resetFishingBoats = function (gameState, sea) {
	if (sea !== undefined)
...
ARCH.NavalManager.prototype.getFishSea = function (gameState, fish) {
...
ARCH.NavalManager.prototype.canFishSafely = function (gameState, fish) {
...
ARCH.NavalManager.prototype.getUnconnectedSeas = function (gameState, region) {
	let seas = gameState.ai.accessibility.regionLinks[region].slice();
	this.docks.forEach(dock => {
		if (!dock.hasClass("Dock") || ARCH.getLandAccess(gameState, dock) !== region)
			return;
		let i = seas.indexOf(ARCH.getSeaAccess(gameState, dock));
		if (i !== -1)
			seas.splice(i--, 1);
	});
	return seas;
...
ARCH.NavalManager.prototype.checkEvents = function (gameState, queues, events) {
...
/// DEBUG
		gameState.ai.logger.push("DEBUG", "NavalManager", "One ship " + shipId + " from plan " + plan.ID + " destroyed during " + plan.state);
/// DEBUG
...
ARCH.NavalManager.prototype.getPlan = function (ID) {
...
ARCH.NavalManager.prototype.addPlan = function (plan) {
 ...
 */
ARCH.NavalManager.prototype.requireTransport = function (gameState, ent, startIndex, endIndex, endPos) {
	if (!ent.canGarrison())
		return false;

	if (ent.getMetadata(PlayerID, "transport") !== undefined) {
/// DEBUG
		gameState.ai.logger.push("ERROR", "NavalManager", "Unit " + ent.id() + " has already requested transport.");
/// DEBUG
		return false;
	}
	...
	for (let plan of this.transportPlans) {
		...
		if (ARCH.isSiegeUnit(ent) && plan.units.filter(unit => ARCH.isSiegeUnit(unit)).length > 3)
			continue;
		plans.push(plan);
	}
	...
	let plan = new ARCH.TransportPlan(gameState, [ent], startIndex, endIndex, endPos);
	if (plan.failed) {
/// DEBUG
		gameState.ai.logger.push("WARNING", "NavalManager", "Transport plan failed!");
/// DEBUG
		return false;
	}
	plan.init(gameState);
...
ARCH.NavalManager.prototype.splitTransport = function (gameState, plan) {
/// DEBUG
	gameState.ai.logger.push("DEBUG", "NavalManager", "Split Transport plan started, splitting units for each transport!");
/// DEBUG
	let newplan = new ARCH.TransportPlan(gameState, [], plan.startIndex, plan.endIndex, plan.endPos);
	if (newplan.failed) {
/// DEBUG
		gameState.ai.logger.push("WARNING", "NavalManager", "Split Transport plan failed!");
/// DEBUG
		return false;
	}
 ...
 */
ARCH.NavalManager.prototype.createTransportIfNeeded = function (gameState, fromPos, toPos, toAccess) {
	...
	for (let ship of this.ships.values()) {
		...
		let plan = new ARCH.TransportPlan(gameState, units, fromAccess, toAccess, toPos, ship);
		if (plan.failed)
			continue;
		plan.init(gameState);
		this.transportPlans.push(plan);
...
ARCH.NavalManager.prototype.setMinimalTransportShips = function (gameState, sea, number) {
	if (!sea)
		return;
...
ARCH.NavalManager.prototype.checkLevels = function (gameState, queues) {
...
ARCH.NavalManager.prototype.maintainFleet = function (gameState, queues) {
	if (queues.ships.hasQueuedUnits())
		return;
	...
	this.updateNavy(gameState);
/// DEBUG
	gameState.ai.logger.push("DEBUG", "NavalManager", "Basic ship counts => Fishing Boats: " + this.fishShips.length + " Transport Ships: " + this.transportShips.length + " War Ships: " + this.warShips.length);
/// DEBUG
	for (let sea = 0; sea < this.seaShips.length; ++sea) {
		...
		if (this.transportShips.length < this.Config.Navy.targetNumTransportships
			&& this.seaTransportShips[sea].length < this.wantedTransportShips[sea]) {
			let template = this.getBestShip(gameState, sea, "transport");
			if (template) {
				queues.ships.addPlan(new ARCH.TrainingPlan(gameState, template, {
					"sea": sea,
					"type": "Transport",
					"ship": true
				}, 1, 1));
		...
		if (this.fishShips.length < this.Config.Navy.targetNumFishers
			&& this.seaFishShips[sea].length < this.wantedFishShips[sea]) {
		...
		if (this.warShips.length < this.Config.Navy.targetNumWarships
			&& this.seaWarShips[sea].length < this.wantedWarShips[sea]) {
...
ARCH.NavalManager.prototype.assignShipsToPlans = function (gameState) {
	for (let plan of this.transportPlans)
...
ARCH.NavalManager.prototype.isShipBoarding = function (ship) {
	if (!ship.position())
		return false;
 ...
 */
ARCH.NavalManager.prototype.moveApart = function (gameState) {
	let blockedShips = [];
	let blockedIds = [];
	...
	for (let ship of this.ships.values()) {
		...
		if (ship.getMetadata(PlayerID, "transporter") !== undefined ||
			...
			if (ARCH.gatherTreasure(gameState, ship, true))
				continue;
			...
			for (let dock of gameState.getAllyStructures().filter(API3.Filters.byClass("Dock")).values()) {
				if (ARCH.getSeaAccess(gameState, dock) !== sea)
					continue;
...
ARCH.NavalManager.prototype.buildNavalStructures = function (gameState, queues) {
	...
	if (queues.dock.countQueuedUnitsWithClass("NavalMarket") === 0 &&
		...
		for (let base of gameState.ai.HQ.baseManagers) {
			...
			for (let sea of remaining) {
				...
				queues.dock.addPlan(new ARCH.ConstructionPlan(gameState, "Dock", "structures/{civ}_dock", {
					"land": wantedLand,
					"sea": sea
				}));
				dockStarted = true;
	...
	queues.militaryBuilding.addPlan(new ARCH.ConstructionPlan(gameState, "Dock", template, {
		"land": wantedLand,
		"sea": sea
	}));
};
...
ARCH.NavalManager.prototype.getBestShip = function (gameState, sea, goal) {
...
ARCH.NavalManager.prototype.update = function (gameState, queues, events) {
///
...
///
	...
	for (let i = 0; i < this.transportPlans.length; ++i) {
		...
		if (remaining)
			continue;
/// DEBUG
		gameState.ai.logger.push("DEBUG", "NavalManager", "No more units on transport plan " + this.transportPlans[i].ID);
/// DEBUG
...
///
ARCH.NavalManager.prototype.Serialize = function () {
	let properties = {
...
ARCH.NavalManager.prototype.Deserialize = function (gameState, data) {
	...
	for (let i in data.transports) {
		...
		let plan = new ARCH.TransportPlan(gameState, [], dataPlan.startIndex, dataPlan.endIndex, dataPlan.endPos);
