let ARCH = function (m) {
	...
	m.NavalManager.prototype.updateNavy = function (gameState, init = false) {
		// docks
...
/// DEBUG
		for (let sea = 0; sea < this.seaShips.length; ++sea) {
			...
			if (this.transportShips.length < this.Config.Navy.targetNumTransportships
				...
				if (template) {
					queues.ships.addPlan(new m.TrainingPlan(gameState, template, {
						"sea": sea,
						"type": "Transport",
						"ship": true
					}, 1, 1));
	...
	m.NavalManager.prototype.buildNavalStructures = function (gameState, queues) {
		...
		if (queues.dock.countQueuedUnitsWithClass("NavalMarket") === 0 &&
			...
			for (let base of gameState.ai.HQ.baseManagers) {
				...
				for (let sea of remaining) {
					...
					queues.dock.addPlan(new m.ConstructionPlan(gameState, "Dock", "structures/{civ}_dock", {
						"land": wantedLand,
						"sea": sea
					}));
					dockStarted = true;
		...
		queues.militaryBuilding.addPlan(new m.ConstructionPlan(gameState, "Dock", template, {
			"land": wantedLand,
			"sea": sea
		}));
	};
