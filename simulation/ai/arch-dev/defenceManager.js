ARCH.DefenceManager = function (Config) {
	...
	this.attackingUnits = {};
	this.attackedAllies = {};
	this.stillDangerous = false;
};

ARCH.DefenceManager.prototype.update = function (gameState, events) {
	///
...
ARCH.DefenceManager.prototype.makeIntoArmy = function (gameState, entityID, type = "default") {
	if (type === "default") {
	...
	let army = new ARCH.DefenceArmy(gameState, [entityID], type);

	this.armies.push(army);
};

ARCH.DefenceManager.prototype.getArmy = function (partOfArmy) {
...
ARCH.DefenceManager.prototype.isDangerous = function (gameState, entity) {
	if (!entity.position())
		return false;
...
ARCH.DefenceManager.prototype.checkEnemyUnits = function (gameState) {
	for (let i = 1; i < gameState.sharedScript.playersData.length; i++) {
		this.checkEnemyPlayerUnits(gameState, i);
	}
};

ARCH.DefenceManager.prototype.checkEnemyPlayerUnits = function (gameState, playerNo) {

	this.attackingUnits[playerNo] = undefined;

	if (playerNo === PlayerID) {
		if (!this.armies.length) {
			// check if we can recover capture points from any of our not decaying structures
			for (let ent of gameState.getOwnStructures().values()) {
				...
				for (let j = 0; j < capture.length; ++j)
					if (gameState.isPlayerEnemy(j))
						lost += capture[j];
				if (lost < Math.ceil(0.25 * capture[playerNo]))
					continue;
		...
		return;
	} else if (!gameState.isPlayerEnemy(playerNo))
		return;

	// loop through enemy units
	for (let ent of gameState.getEnemyUnits(playerNo).values()) {
	...
	if (playerNo !== 0 || this.armies.length > 1 || gameState.ai.HQ.numActiveBases() === 0)
		return;
	...
	for (let ent of gameState.getEnemyStructures(playerNo).values()) {
...
ARCH.DefenceManager.prototype.checkEnemyArmies = function (gameState) {
	...
	for (let i = 0; i < this.armies.length; ++i) {
		...
		for (let base of bases.values()) {
			...
			if (API3.SquareVectorDistance(base.position(), army.foePosition) > 40000)
				continue;
/// DEBUG
			gameState.ai.logger.push("WARNING", "DefenceManager", "Army in neutral territory, but still near one of our Civil Centre");
/// DEBUG
			this.stillDangerous = true;
			break;
		}
		if (this.stillDangerous)
			continue;
...
ARCH.DefenceManager.prototype.assignDefenders = function (gameState) {
	if (!this.armies.length)
		return;
	...
	for (let army of this.armies) {
		...
		for (let entId of army.foeEntities) {
			...
			if (!ent || !ent.position())
				continue;
			armyAccess = ARCH.getLandAccess(gameState, ent);
			break;
		}
/// DEBUG
		if (!armyAccess) {
			gameState.ai.logger.push("ERROR", "DefenceManager", "Attacking army " + army.ID + " without access");
		}
/// DEBUG
		army.recalculatePosition(gameState);
	...
	for (let ipass = 0; ipass < 2; ++ipass) {
		...
		for (let i = 0; i < potentialDefenders.length; ++i) {
			...
			let aMin;
			let distMin;
			let access = ipass === 0 ? ARCH.getLandAccess(gameState, ent) : undefined;
			...
			if (backup < Math.max(12, gameState.getPopulation() * 0.1) && (aMin === undefined || distMin > 40000 &&
			...
			armiesNeeding[aMin].need -= ARCH.getMaxStrength(ent);
	...
	gameState.ai.HQ.trainingManager.emergencyDefender(gameState, armiesPos);
};

ARCH.DefenceManager.prototype.abortArmy = function (gameState, army) {
	army.clear(gameState);
	for (let i = 0; i < this.armies.length; ++i) {
...
/**
 * If our defence structures are attacked, garrison soldiers inside when possible
 * and if a support unit is attacked and has less than 55% health, garrison it inside the nearest healing structure
 * and if a ranged siege unit (not used for defence) is attacked, garrison it in the nearest fortress
 * If our hero is attacked with regicide victory condition, the victoryManager will handle it
 */
ARCH.DefenceManager.prototype.checkEvents = function (gameState, events) {
	...
	for (let evt of events.OwnershipChanged)   // capture events
	{
		if (gameState.isPlayerMutualAlly(evt.from) && evt.to > 0) {
			...
			if (ent && ent.hasClass("CivCentre")) // one of our cc has been captured
				gameState.ai.HQ.attackManager.switchDefenceToAttack(gameState, ent, {"range": 150});
		}
	}
	...
	for (let evt of events.Attacked) {
		...
		// Signal this attacker to our defence manager, except if we are in enemy territory
		...
		if (target.getMetadata(PlayerID, "PartOfArmy") !== undefined) {
			...
			if (army && army.getType() === "capturing") {
				let abort = false;
		...
		if (target.hasClass("Unit") && attacker.hasClass("Unit")) {
			...
			if (currentTarget) {
				...
				if (unitAIStateOrder === "REPAIR" && currentTarget.hasDefensiveFire())
					continue;
				if (unitAIStateOrder === "COMBAT" && !ARCH.isSiegeUnit(currentTarget) &&
					...
					if (minEnt) {
						...
						minEnt.attack(attacker.id(), ARCH.allowCapture(gameState, minEnt, attacker));
					}
				}
			}
			target.attack(attacker.id(), ARCH.allowCapture(gameState, target, attacker));
		}
	}
};

ARCH.DefenceManager.prototype.garrisonUnitsInside = function (gameState, target, data) {
	...
	let access = ARCH.getLandAccess(gameState, target);
	...
	if (allowMelee === undefined) {
		...
		if (data.attacker)
			allowMelee = data.attacker.hasClass("Structure") ? data.attacker.attackRange("Ranged") : !ARCH.isSiegeUnit(data.attacker);
		else
			allowMelee = true;
	}
	let units = gameState.getOwnUnits().filter(ent => {
		...
		return ARCH.getLandAccess(gameState, ent) === access;
...
ARCH.DefenceManager.prototype.garrisonSiegeUnit = function (gameState, unit) {
	let distmin = Math.min();
	let nearest;
	let unitAccess = ARCH.getLandAccess(gameState, unit);
	let garrisonManager = gameState.ai.HQ.garrisonManager;
	for (let ent of gameState.getAllyStructures().values()) {
		...
		if (ent.hitpoints() < ent.garrisonEjectHealth() * ent.maxHitpoints())
			continue;
		if (ARCH.getLandAccess(gameState, ent) !== unitAccess)
			continue;
 ...
 */
ARCH.DefenceManager.prototype.garrisonAttackedUnit = function (gameState, unit, emergency = false) {
	let distmin = Math.min();
	let nearest;
	let unitAccess = ARCH.getLandAccess(gameState, unit);
	let garrisonManager = gameState.ai.HQ.garrisonManager;
 ...
 * @return {number}
 */
ARCH.DefenceManager.prototype.GetCooperationLevel = function (ally) {
...
/**
 * Switch a defence army into an attack if needed
 */
ARCH.DefenceManager.prototype.switchToAttack = function (gameState, army) {
	if (!army)
		return;
	for (let targetId of this.targetList) {
		let target = gameState.getEntityById(targetId);
		if (!target || !target.position() || !gameState.isPlayerEnemy(target.owner()))
			continue;
		let targetAccess = ARCH.getLandAccess(gameState, target);
		let targetPos = target.position();
		for (let entId of army.ownEntities) {
			let ent = gameState.getEntityById(entId);
			if (!ent || !ent.position() || ARCH.getLandAccess(gameState, ent) !== targetAccess)
				continue;
			if (API3.SquareVectorDistance(targetPos, ent.position()) > 14400)
				continue;
			gameState.ai.HQ.attackManager.switchDefenceToAttack(gameState, target, {
				"armyID": army.ID,
				"uniqueTarget": true
			});
			return;
...
///
ARCH.DefenceManager.prototype.Serialize = function () {
	let properties = {
...
ARCH.DefenceManager.prototype.Deserialize = function (gameState, data) {
	for (let key in data.properties)
		this[key] = data.properties[key];

	this.armies = [];
	for (let dataArmy of data.armies) {
		let army = new ARCH.DefenceArmy(gameState, []);
