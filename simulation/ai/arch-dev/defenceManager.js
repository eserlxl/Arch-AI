let ARCH = function (m) {
	...
	m.DefenceManager = function (Config) {
	...
	m.DefenceManager.prototype.update = function (gameState, events) {
		///
	...
	m.DefenceManager.prototype.makeIntoArmy = function (gameState, entityID, type = "default") {
		if (type === "default") {
		...
		let army = new m.DefenceArmy(gameState, [entityID], type);

		this.armies.push(army);
	};

	m.DefenceManager.prototype.getArmy = function (partOfArmy) {
	...
	m.DefenceManager.prototype.isDangerous = function (gameState, entity) {
		if (!entity.position())
			return false;
	...
	m.DefenceManager.prototype.checkEnemyUnits = function (gameState) {
		for (let i = 1; i < gameState.sharedScript.playersData.length; i++) {
			this.checkEnemyPlayerUnits(gameState, i);
		}
	};

	m.DefenceManager.prototype.checkEnemyPlayerUnits = function (gameState, playerNo) {
	...
	m.DefenceManager.prototype.checkEnemyArmies = function (gameState) {
...
/// DEBUG
	...
	m.DefenceManager.prototype.assignDefenders = function (gameState) {
		if (!this.armies.length)
			return;
...
/// DEBUG
	...
	m.DefenceManager.prototype.abortArmy = function (gameState, army) {
		army.clear(gameState);
		for (let i = 0; i < this.armies.length; ++i) {
	 ...
	 */
	m.DefenceManager.prototype.checkEvents = function (gameState, events) {
	...
	m.DefenceManager.prototype.garrisonUnitsInside = function (gameState, target, data) {
		...
		let units = gameState.getOwnUnits().filter(ent => {
			...
			return m.getLandAccess(gameState, ent) === access;
	...
	m.DefenceManager.prototype.garrisonSiegeUnit = function (gameState, unit) {
	 ...
	 */
	m.DefenceManager.prototype.garrisonAttackedUnit = function (gameState, unit, emergency = false) {
		let distmin = Math.min();
		let nearest;
		let unitAccess = m.getLandAccess(gameState, unit);
		let garrisonManager = gameState.ai.HQ.garrisonManager;
	 ...
	 * @return {number}
	 */
	m.DefenceManager.prototype.GetCooperationLevel = function (ally) {
	 ...
	 */
	m.DefenceManager.prototype.switchToAttack = function (gameState, army) {
		if (!army)
			return;
		for (let targetId of this.targetList) {
			let target = gameState.getEntityById(targetId);
...
///
	m.DefenceManager.prototype.Serialize = function () {
		let properties = {
	...
	m.DefenceManager.prototype.Deserialize = function (gameState, data) {
		for (let key in data.properties)
			this[key] = data.properties[key];

		this.armies = [];
		for (let dataArmy of data.armies) {
			let army = new m.DefenceArmy(gameState, []);
