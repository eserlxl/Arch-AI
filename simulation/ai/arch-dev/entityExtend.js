...
ARCH.getMaxStrength = function (ent, againstClass) {
	let strength = 0;
	...
	for (let type of attackTypes) {
		...
		for (let str in attackStrength) {
			...
			switch (str) {
				...
				default:
/// DEBUG
					gameState.ai.logger.push("DEBUG", "EntityExtend", "Unknown attackStrength in getMaxStrength: " + str);
/// DEBUG
			}
		}
		...
		for (let str in attackTimes) {
			...
			switch (str) {
				...
				default:
/// DEBUG
					gameState.ai.logger.push("DEBUG", "EntityExtend", "Unknown attackTimes in getMaxStrength: " + str);
/// DEBUG
			}
		}
	}
	...
	for (let str in armourStrength) {
		...
		switch (str) {
			...
			default:
/// DEBUG
				gameState.ai.logger.push("DEBUG", "EntityExtend", "Unknown armourStrength in getMaxStrength: " + str);
/// DEBUG
		}
	}
...
ARCH.getLandAccess = function (gameState, ent) {
	if (!ent.position())
		return false;
...
ARCH.getSeaAccess = function (gameState, ent) {
...
ARCH.setSeaAccess = function (gameState, ent) {
	ARCH.getSeaAccess(gameState, ent);
};

/** Decide if we should try to capture (returns true) or destroy (return false) */
ARCH.allowCapture = function (gameState, ent, target) {
	...
	if (!capturableTargets.has(target.id())) {
		capture = ent.captureStrength() * ARCH.getAttackBonus(ent, target, "Capture");
		...
		if (!capturable.ents.has(ent.id())) {
			capturable.strength += ent.captureStrength() * ARCH.getAttackBonus(ent, target, "Capture");
...
ARCH.getAttackBonus = function (ent, target, type) {
	let attackBonus = 1;
...
ARCH.returnResources = function (gameState, ent) {
	...
	for (let dropsite of dropsiteCollection.values()) {
		...
		if (owner !== PlayerID && (!dropsite.isSharedDropsite() || !gameState.isPlayerMutualAlly(owner)))
			continue;
		if (ARCH.getLandAccess(gameState, dropsite) !== access)
			continue;
 ...
 * @return {boolean}
 */
ARCH.IsSupplyFull = function (gameState, ent) {
 ...
 */
ARCH.getBestBase = function (gameState, ent, onlyConstructedBase = false, exclude = false) {
	let pos = ent.position();
	let accessIndex;
	if (!pos) {
		let holder = ARCH.getHolder(gameState, ent);
		if (!holder || !holder.position()) {
/// DEBUG
			gameState.ai.logger.push("ERROR", "EntityExtend", "Entity without position, but not garrisoned");
			ARCH.dumpEntity(ent)
/// DEBUG
			return gameState.ai.HQ.baseManagers[0];
		}
		pos = holder.position();
		accessIndex = ARCH.getLandAccess(gameState, holder);
	} else
		accessIndex = ARCH.getLandAccess(gameState, ent);
...
ARCH.getHolder = function (gameState, ent) {
...
ARCH.getBuiltEntity = function (gameState, ent) {
 ...
 */
ARCH.isNotWorthBuilding = function (gameState, ent) {
 ...
 */
ARCH.isLineInsideEnemyTerritory = function (gameState, pos1, pos2, step = 70) {
...
ARCH.gatherTreasure = function (gameState, ent, water = false) {
	...
	let treasureFound;
	let distmin = Math.min();
	let access = water ? ARCH.getSeaAccess(gameState, ent) : ARCH.getLandAccess(gameState, ent);
	for (let treasure of gameState.ai.HQ.treasures.values()) {
		if (ARCH.IsSupplyFull(gameState, treasure))
			continue;
		...
		if (lastGathered && gameState.ai.elapsedTime - lastGathered < 20)
			continue;
		if (!water && access !== ARCH.getLandAccess(gameState, treasure))
			continue;
		if (water && access !== ARCH.getSeaAccess(gameState, treasure))
			continue;
