 ...
 */
ARCH.DiplomacyManager = function (Config) {
	this.Config = Config;
 ...
 */
ARCH.DiplomacyManager.prototype.init = function (gameState) {
 ...
 */
ARCH.DiplomacyManager.prototype.tributes = function (gameState) {
			...
			if (donor && availableResources[res] > 200 && allyResources[res] < 0.2 * availableResources[res]) {
				...
				for (let k = 0; k < 2; ++k) {
					if (mostNeeded[k].type === res && mostNeeded[k].wanted > 0) {
						...
						ARCH.chatRequestTribute(gameState, res);
/// DEBUG
						gameState.ai.logger.push("DEBUG", "DiplomacyManager", "Tribute on " + res + " requested to player " + i);
/// DEBUG
						break;
					}
				}
...
/// DEBUG
		gameState.ai.logger.push("DEBUG", "DiplomacyManager", "Tribute " + uneval(tribute) + " sent to player " + i);
/// DEBUG
		if (this.Config.chat)
			ARCH.chatSentTribute(gameState, i);
		Engine.PostCommand(PlayerID, {"type": "tribute", "player": i, "amounts": tribute});
	}
};

ARCH.DiplomacyManager.prototype.checkEvents = function (gameState, events) {
	...
	for (let evt of events.TributeExchanged) {
		if (evt.to === PlayerID && !gameState.isPlayerAlly(evt.from) && this.receivedDiplomacyRequests.has(evt.from)) {
			...
			if (request.status === "waitingForTribute") {
				...
				if (request.wanted <= 0) {
/// DEBUG
					gameState.ai.logger.push("DEBUG", "DiplomacyManager", "Player " + uneval(evt.from) + " has sent the required tribute amount");
/// DEBUG
	...
	for (let evt of events.DiplomacyChanged) {
		...
		if (request !== undefined &&
			...
			ARCH.chatAnswerRequestDiplomacy(gameState, evt.player, "ally", response);
...
/// DEBUG
		gameState.ai.logger.push("DEBUG", "DiplomacyManager", "Responding to diplomacy request from AI player " + evt.source + " with " + uneval(request));
/// DEBUG
...
/// DEBUG
			gameState.ai.logger.push("DEBUG", "DiplomacyManager", "Responding to tribute request from AI player " + evt.source + " with " + uneval(responseTribute));
/// DEBUG
 ...
 */
ARCH.DiplomacyManager.prototype.lastManStandingCheck = function (gameState) {
	...
	if (playerToTurnAgainst) {
		...
		if (request && request.status !== "allianceBroken") {
			if (request.status === "waitingForTribute")
				ARCH.chatAnswerRequestDiplomacy(gameState, player, request.requestType, "decline");
 ...
 */
ARCH.DiplomacyManager.prototype.handleDiplomacyRequest = function (gameState, player, requestType) {
	...
	ARCH.chatAnswerRequestDiplomacy(gameState, player, requestType, response, requiredTribute);
};

ARCH.DiplomacyManager.prototype.changePlayerDiplomacy = function (gameState, player, newDiplomaticStance) {
...
/// DEBUG
	gameState.ai.logger.push("INFO", "DiplomacyManager", "Diplomacy stance with player " + player + " is now " + newDiplomaticStance);
/// DEBUG
	if (this.Config.chat)
		ARCH.chatNewDiplomacy(gameState, player, newDiplomaticStance);
};

ARCH.DiplomacyManager.prototype.checkRequestedTributes = function (gameState) {
	for (let [player, data] of this.receivedDiplomacyRequests)
		if (data.status === "waitingForTribute" && gameState.ai.elapsedTime > data.warnTime) {
			if (data.sentWarning) {
				...
				ARCH.chatAnswerRequestDiplomacy(gameState, player, data.requestType, "decline");
			} else {
				data.sentWarning = true;
				data.warnTime = gameState.ai.elapsedTime + 60;
				ARCH.chatAnswerRequestDiplomacy(gameState, player, data.requestType, "waitingForTribute", {
					"wanted": data.wanted,
					"type": data.type
				});
			}
 ...
 */
ARCH.DiplomacyManager.prototype.sendDiplomacyRequest = function (gameState) {
	let player;
	let max = 0;
	for (let i = 1; i < gameState.sharedScript.playersData.length; ++i) {
		let mutualEnemies = 0;
...
/// DEBUG
	gameState.ai.logger.push("DEBUG", "DiplomacyManager", "Sending diplomacy request to player " + player + " with " + requestType);
/// DEBUG
	Engine.PostCommand(PlayerID, {
		"type": "diplomacy-request",
		"source": PlayerID,
		"player": player,
		"to": requestType
	});
	ARCH.chatNewRequestDiplomacy(gameState, player, requestType, "sendRequest");
};

ARCH.DiplomacyManager.prototype.checkSentDiplomacyRequests = function (gameState) {
	for (let [player, data] of this.sentDiplomacyRequests)
		if (gameState.ai.elapsedTime > data.timeSent + 60 && !gameState.ai.HQ.saveResources &&
			...
			ARCH.chatNewRequestDiplomacy(gameState, player, data.requestType, "requestExpired");
			this.sentDiplomacyRequests.delete(player);
		}
};

ARCH.DiplomacyManager.prototype.update = function (gameState, events) {
...
///
ARCH.DiplomacyManager.prototype.Serialize = function () {
	return {
...
ARCH.DiplomacyManager.prototype.Deserialize = function (data) {
	for (let key in data)
		this[key] = data[key];
};
///
