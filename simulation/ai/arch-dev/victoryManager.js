...
ARCH.VictoryManager = function (Config) {
	this.Config = Config;
 ...
 */
ARCH.VictoryManager.prototype.init = function (gameState) {
 ...
 */
ARCH.VictoryManager.prototype.checkEvents = function (gameState, events) {
	if (gameState.getVictoryConditions().has("wonder")) {
		...
		for (let evt of events.Attacked) {
			...
			if (plan !== -2 && plan !== -3) {
				...
				if (target.getMetadata(PlayerID, "PartOfArmy")) {
					let army = gameState.ai.HQ.defenceManager.getArmy(target.getMetadata(PlayerID, "PartOfArmy"));
					if (army)
...
ARCH.VictoryManager.prototype.removeCriticalEnt = function (gameState, criticalEntId) {
 ...
 */
ARCH.VictoryManager.prototype.manageCriticalEntHealers = function (gameState, queues) {
	...
	for (let data of this.criticalEnts.values()) {
		...
		queues.healer.addPlan(new ARCH.TrainingPlan(gameState, template, {
			"role": "criticalEntHealer",
			"base": 0
		}, 1, 1));
		return;
	}
 ...
 */
ARCH.VictoryManager.prototype.manageCriticalEntGuards = function (gameState) {
	...
	for (let [id, data] of this.criticalEnts) {
		...
		for (let checkForSameAccess of [true, false]) {
			...
			for (let entity of gameState.getOwnUnits().filter(API3.Filters.byMetadata(PlayerID, "plan", -1))) {
				if (!this.tryAssignMilitaryGuard(gameState, entity, criticalEnt, checkForSameAccess))
					continue;
				--numWorkers;
				if (++data.guardsAssigned >= militaryGuardsPerCriticalEnt || numWorkers <= minWorkers + deltaWorkers * data.guardsAssigned)
...
ARCH.VictoryManager.prototype.tryAssignMilitaryGuard = function (gameState, guardEnt, criticalEnt, checkForSameAccess) {
	if (guardEnt.getMetadata(PlayerID, "plan") !== undefined ||
		...
		checkForSameAccess && (!guardEnt.position() || !criticalEnt.position() ||
			ARCH.getLandAccess(gameState, criticalEnt) !== ARCH.getLandAccess(gameState, guardEnt)))
		return false;
...
ARCH.VictoryManager.prototype.pickCriticalEntRetreatLocation = function (gameState, criticalEnt, emergency) {
	gameState.ai.HQ.defenceManager.garrisonAttackedUnit(gameState, criticalEnt, emergency);
	...
	let accessIndex = ARCH.getLandAccess(gameState, criticalEnt);
	let bestBase = ARCH.getBestBase(gameState, criticalEnt, true);
 ...
 */
ARCH.VictoryManager.prototype.assignGuardToCriticalEnt = function (gameState, guardEnt, criticalEntId) {
	...
	let guardEntAccess = ARCH.getLandAccess(gameState, guardEnt);
	let criticalEntAccess = ARCH.getLandAccess(gameState, criticalEnt);
	if (guardEntAccess === criticalEntAccess) {
		let queued = ARCH.returnResources(gameState, guardEnt);
...
ARCH.VictoryManager.prototype.resetCaptureGaiaRelic = function (gameState) {
	// Do not capture gaia relics too frequently as the ai has access to the entire map
	this.tryCaptureGaiaRelicLapseTime = Math.max(90, gameState.ai.elapsedTime + this.Config.Military.relicSearchLatency);
	this.tryCaptureGaiaRelic = false;
};

ARCH.VictoryManager.prototype.update = function (gameState, events, queues) {
	...
	if (!gameState.getVictoryConditions().has("wonder") && !gameState.getVictoryConditions().has("regicide") &&
 ...
 */
ARCH.VictoryManager.prototype.captureGaiaRelic = function (gameState, relic) {
	...
	for (let plan of plans) {
		...
		for (let ent of attack.unitCollection.values())
			capture += ent.captureStrength() * ARCH.getAttackBonus(ent, relic, "Capture");
	}
	// No need to make a new attack if already enough units
	if (capture > sumCapturePoints / 50)
		return;
	let relicPosition = relic.position();
	let access = ARCH.getLandAccess(gameState, relic);
...
ARCH.VictoryManager.prototype.abortCaptureGaiaRelic = function (gameState, relicId) {
...
///
ARCH.VictoryManager.prototype.Serialize = function () {
	return {
...
ARCH.VictoryManager.prototype.Deserialize = function (data) {
	for (let key in data)
		this[key] = data[key];
};
///
