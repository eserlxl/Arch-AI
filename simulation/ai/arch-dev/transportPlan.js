...
ARCH.TransportPlan = function (gameState, units, startIndex, endIndex, endPos, ship) {
	...
	if (startIndex === 1) {
		...
		if (!this.sea) {
			this.failed = true;
/// DEBUG
			gameState.ai.logger.push("DEBUG", "TransportPlan", "Transport plan with bad path: startIndex " + startIndex + " endIndex " + endIndex);
/// DEBUG
			return false;
		}
	}
...
/// DEBUG
	gameState.ai.logger.push("DEBUG", "TransportPlan", "Starting a new transport plan with ID " + this.ID +
...
ARCH.TransportPlan.prototype.init = function (gameState) {
...
ARCH.TransportPlan.prototype.countFreeSlots = function () {
	let slots = 0;
...
ARCH.TransportPlan.prototype.countFreeSlotsOnShip = function (ship) {
...
ARCH.TransportPlan.prototype.assignUnitToShip = function (gameState, ent) {
...
ARCH.TransportPlan.prototype.assignShip = function (gameState) {
	let pos;
...
ARCH.TransportPlan.prototype.addUnit = function (unit, endPos) {
...
ARCH.TransportPlan.prototype.removeUnit = function (gameState, unit) {
...
ARCH.TransportPlan.prototype.releaseShip = function (ship) {
	if (ship.getMetadata(PlayerID, "transporter") !== this.ID) {
/// DEBUG
		gameState.ai.logger.push("BUG", "TransportPlan", "Trying to remove a transporter ship with " + ship.getMetadata(PlayerID, "transporter") +
...
ARCH.TransportPlan.prototype.releaseAll = function () {
...
ARCH.TransportPlan.prototype.cancelTransport = function (gameState) {
	...
	if (!base.anchor || !base.anchor.position()) {
		...
		this.units.forEach(unit => {
			unit.setMetadata(PlayerID, "base", base.ID);
		});
	}
...
ARCH.TransportPlan.prototype.update = function (gameState) {
...
ARCH.TransportPlan.prototype.onBoarding = function (gameState) {
	let ready = true;
...
/// DEBUG
						gameState.ai.logger.push("DEBUG", "TransportPlan", "New attempt to find a landing point for the ship " + shipId);
/// DEBUG
...
/// DEBUG
							gameState.ai.logger.push("WARNING", "TransportPlan", "Unit was blocked and there is no way to move away. Destroying the unit!");
/// DEBUG
...
ARCH.TransportPlan.prototype.isOnBoard = function (ent) {
	for (let ship of this.transportShips.values()) {
...
/** when avoidEnemy is true, we try to not board/unboard in ennemy territory */
ARCH.TransportPlan.prototype.getBoardingPos = function (gameState, ship, landIndex, seaIndex, destination, avoidEnnemy) {
	if (!gameState.ai.HQ.navalManager.landingZones[landIndex]) {
/// DEBUG
		gameState.ai.logger.push("WARNING", "TransportPlan", "No landing zone for land " + landIndex);
/// DEBUG
		return destination;
	} else if (!gameState.ai.HQ.navalManager.landingZones[landIndex][seaIndex]) {
/// DEBUG
		gameState.ai.logger.push("WARNING", "TransportPlan", "No landing zone for land " + landIndex + " and sea " + seaIndex);
/// DEBUG
		return destination;
	}
...
ARCH.TransportPlan.prototype.onSailing = function (gameState) {
...
/// DEBUG
		gameState.ai.logger.push("WARNING", "TransportPlan", "Reloading failed for transport " + this.ID);
/// DEBUG
		// destroy the unit if inaccessible otherwise leave it there
		let index = ARCH.getLandAccess(gameState, ent);
		if (gameState.ai.HQ.landRegions[index]) {
/// DEBUG
			gameState.ai.logger.push("DEBUG", "TransportPlan", "Recovered entity was kept: " + ent.id());
/// DEBUG
			this.resetUnit(gameState, ent);
...
/// DEBUG
			gameState.ai.logger.push("DEBUG", "TransportPlan", "Recovered entity was destroyed: " + ent.id());
/// DEBUG
			this.resetUnit(gameState, ent);
			ent.destroy();
		}
...
/// DEBUG
					gameState.ai.logger.push("ERROR", "TransportPlan", "Unit is not on the ship, but without position ???");
/// DEBUG
					this.resetUnit(gameState, ent);
					ent.destroy();
				}
...
/// DEBUG
				gameState.ai.logger.push("ERROR", "TransportPlan", "Unit is on the ship, but there is no ship ???");
/// DEBUG
				this.resetUnit(gameState, ent);
				ent.destroy();
			}
		} else if (ARCH.getLandAccess(gameState, ent) !== this.endIndex) {
			// unit unloaded on a wrong region - try to regarrison it and move a bit the ship
/// DEBUG
			gameState.ai.logger.push("DEBUG", "TransportPlan", "Unit was unloaded on a wrong region, trying to garrison it again.");
/// DEBUG
			let ship = gameState.getEntityById(ent.getMetadata(PlayerID, "onBoard"));
...
/// DEBUG
				gameState.ai.logger.push("WARNING", "TransportPlan", "No way! Destroying the unit!");
/// DEBUG
				this.resetUnit(gameState, ent);
				ent.destroy();
			}
	...
	for (let ship of this.transportShips.values()) {
		...
		if (dist > this.boardingRange) {
			...
			if (this.nTry[shipId] > 2)	// we must have been blocked by something ... try with another boarding point
			{
				this.nTry[shipId] = 0;
/// DEBUG
				gameState.ai.logger.push("WARNING", "TransportPlan", "New attempt to find a landing point! Ship: " + shipId);
/// DEBUG
...
ARCH.TransportPlan.prototype.resetUnit = function (gameState, ent) {
	ent.setMetadata(PlayerID, "transport", undefined);
	ent.setMetadata(PlayerID, "onBoard", undefined);
	ent.setMetadata(PlayerID, "endPos", undefined);
	...
	if (ent.getMetadata(PlayerID, "PartOfArmy")) {
		let army = gameState.ai.HQ.defenceManager.getArmy(ent.getMetadata(PlayerID, "PartOfArmy"));
		if (army)
			army.removeOwn(gameState, ent.id());
	}
};
///
ARCH.TransportPlan.prototype.Serialize = function () {
	return {
		"ID": this.ID,
...
ARCH.TransportPlan.prototype.Deserialize = function (data) {
	for (let key in data)
		this[key] = data[key];
