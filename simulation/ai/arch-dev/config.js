...
let ARCH = function (m) {
	...
	m.Config = function (difficulty, behavior) {
		...
		if (this.difficulty < 3) {
			for (let hash in this.priorities) {
				this.priorities[hash] = m.limit(this.priorities[hash] - Math.pow(4 - this.difficulty, 3) * randIntExclusive(25, 50), 50, 1000);
			}
		}
	...
	m.Config.prototype.setConfig = function (gameState) {
		...
		this.Personality.aggressive = m.interpolate(Math.sqrt(this.popRatio), 0, min, 1, max);
		this.Personality.defensive = m.interpolate(Math.square(1 - this.popRatio), 0, 1 - max, 1, 1 - min);
		...
		this.Personality.cooperative = m.limit(this.Personality.cooperative, 0.15, 0.85);
		/// Admiral
		...
		this.Military.razeLostBuildings = defaultRazeLostBuildingsDecision;
		this.Personality.cooperative = m.limit(this.Personality.cooperative, 0.25, 0.75);
		/// Capitalist
		...
		this.Military.razeLostBuildings = defaultRazeLostBuildingsDecision;
		this.Personality.cooperative = m.limit(this.Personality.cooperative, 0.35, 1);
		/// Imperialist
		...
		this.Military.razeLostBuildings = defaultRazeLostBuildingsDecision;
		this.Personality.cooperative = m.limit(this.Personality.cooperative, 0.5, 1);
		/// Mercantilist
		...
		this.Military.razeLostBuildings = false;
		this.Personality.cooperative = m.limit(this.Personality.cooperative, 0, 0.5);
		/// Patriot
		...
		this.Military.razeLostBuildings = false;
		this.Personality.cooperative = m.limit(this.Personality.cooperative, 0, 0.25);
		/// Unitary
		...
		this.Military.razeLostBuildings = false;
		this.Personality.cooperative = m.limit(this.Personality.cooperative, 0.38, 0.76);
		/// Theocrat
