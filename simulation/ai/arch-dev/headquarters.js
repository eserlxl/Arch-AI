let ARCH = function (m) {
	...
	m.HQ = function (Config) {
		...
		this.defenceManager = new m.DefenceManager(this.Config);
		...
		this.resourceManager = new m.ResourceManager(this.Config);
		...
		this.constructManager = new m.ConstructManager(this.Config);
		this.trainingManager = new m.TrainingManager(this.Config);
...
/// DEBUG
	 ...
	 */
	m.HQ.prototype.pickMostNeededResources = function (gameState, requiredResource = false) {
...
/// DEBUG
	...
	m.HQ.prototype.buildWonder = function (gameState, queues, force = false) {
		...
		queues.wonder.addPlan(new m.ConstructionPlan(gameState, "Wonder", "structures/{civ}_wonder"));
	};
...
/// Unitary
		...
		this.constructManager.target["Colony"] = m.limit(this.constructManager.target["Colony"] + 0.1, 0, 10);
		this.constructManager.target["CivCentre"] = m.limit(this.constructManager.target["CivCentre"] + 0.1, 0, 10);
/// DEBUG
...
///
		...
		if (m.modCheck(gameState.ai.playedTurn, this.Config.Period.base)) {
...
///
		...
		if (m.modCheck(gameState.ai.playedTurn, this.Config.Period.construction)) {
...
///
		...
		if (m.modCheck(gameState.ai.playedTurn, this.Config.Period.research)) {
			if (!this.phasing)
...
///
		...
		if (m.modCheck(gameState.ai.playedTurn, this.Config.Period.training)) {
...
///
		...
		if (m.modCheck(gameState.ai.playedTurn, this.Config.Period.expansion)) {
...
///
		...
		if (m.modCheck(gameState.ai.playedTurn, this.Config.Period.trading)) {
...
///
		...
		if (m.modCheck(gameState.ai.playedTurn, this.Config.Period.defence)) {
...
///
		...
		if (m.modCheck(gameState.ai.playedTurn, this.Config.Period.military)) {
...
///
		...
		if (m.modCheck(gameState.ai.playedTurn, this.Config.Period.navy)) {
...
///
		...
		if (m.modCheck(gameState.ai.playedTurn, this.Config.Period.diplomacy)) {
...
///
		...
		if (m.modCheck(gameState.ai.playedTurn, this.Config.Period.victory)) {
...
///
	...
	m.HQ.prototype.Deserialize = function (gameState, data) {
		...
		this.constructManager = new m.ConstructManager();
