...
ARCH.AttackManager = function (Config) {
	this.Config = Config;
...
ARCH.AttackManager.prototype.init = function (gameState) {
};

/*
 * Check for any structure in range from within our territory, and bomb it
 */
ARCH.AttackManager.prototype.assignBombers = function (gameState) {
 ...
 */
ARCH.AttackManager.prototype.update = function (gameState, queues, events) {
...
/// DEBUG
					if (this.Config.chat)
						ARCH.chatLaunchAttack(gameState, attack.targetPlayer, attack.getType());
...
/// DEBUG
	...
	if (allowAttack) {
		let attackPlan = new ARCH.AttackPlan(gameState, this.Config, this.totalNumber, attack);
...
/// DEBUG
	...
	if (ARCH.modCheck(gameState.ai.playedTurn, this.Config.Defence.bombingAttackPeriod)) {
		this.assignBombers(gameState);
	}
};

ARCH.AttackManager.prototype.getAttackInPreparation = function (type) {
...
ARCH.AttackManager.prototype.cancelAttacksAgainstPlayer = function (gameState, player) {
	for (let attackType in this.upcomingAttacks)
...
ARCH.AttackManager.prototype.raidTargetEntity = function (gameState, ent) {
	let data = {"target": ent};
	let attackPlan = new ARCH.AttackPlan(gameState, this.Config, this.totalNumber, "Raid", data);
 ...
 */
ARCH.AttackManager.prototype.switchDefenceToAttack = function (gameState, target, data) {
...
/// DEBUG
	...
	let attackType = "Rush";
	let attackPlan = new ARCH.AttackPlan(gameState, this.Config, this.totalNumber, attackType, attackData);
	if (attackPlan.failed) {
/// DEBUG
	...
	for (let army of gameState.ai.HQ.defenceManager.armies) {
		...
		while (army.ownEntities.length > 0) {
			...
			let accessOk = unit.getMetadata(PlayerID, "transport") !== undefined ||
				unit.position() && ARCH.getLandAccess(gameState, unit) === targetAccess;
...
///
ARCH.AttackManager.prototype.Serialize = function () {
	let properties = {
...
ARCH.AttackManager.prototype.Deserialize = function (gameState, data) {
	...
	for (let key in data.upcomingAttacks) {
		...
		for (let dataAttack of data.upcomingAttacks[key]) {
			let attack = new ARCH.AttackPlan(gameState, this.Config, dataAttack.properties.name);
